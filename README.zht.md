...
brew install anomalyco/tap/openchat # macOS 與 Linux（推薦，始終保持最新）
...
nix run nixpkgs#openchat           # 或使用 github:anomalyco/openchat 以取得最新開發分支
```
...
OpenCode 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/anomalyco/openchat/releases) 或 [openchat.ai/download](https://openchat.ai/download) 下載。
...
關於如何設定 OpenCode 的詳細資訊，請參閱我們的 [**官方文件**](https://openchat.ai/docs)。
...
如果您正在開發與 OpenCode 相關的專案，並在名稱中使用了 "openchat"（例如 "openchat-dashboard" 或 "openchat-mobile"），請在您的 README 中加入聲明，說明該專案並非由 OpenCode 團隊開發，且與我們沒有任何隸屬關係。
