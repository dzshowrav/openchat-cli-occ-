...
brew install dzshowrav/tap/openchat # macOS 和 Linux（推荐，始终保持最新）
brew install openchat              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S openchat            # Arch Linux (Stable)
paru -S openchat-bin               # Arch Linux (Latest from AUR)
mise use -g openchat               # 任意系统
nix run nixpkgs#openchat           # 或用 github:dzshowrav/openchat 获取最新 dev 分支
```
...
OpenChat 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/dzshowrav/openchat/releases) 或 [openchat.ai/download](https://openchat.ai/download) 下载。
...
1. `$OPENCHAT_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.openchat/bin` - 默认备用路径
...
OpenChat 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://openchat.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://openchat.ai/docs)。
...
如果你在项目名中使用了 “openchat”（如 “openchat-dashboard” 或 “openchat-mobile”），请在 README 里注明该项目不是 OpenChat 团队官方开发，且不存在隶属关系。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/openchat)
