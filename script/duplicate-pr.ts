...
import { createOpenchat } from "@openchat-ai/sdk"
...
`)
  ...
  const openchat = await createOpenchat({ port: 0 })
  ...
  try {
    ...
    const session = await openchat.client.session.create()
    const result = await openchat.client.session
      .prompt({
