...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "openchat/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".openchat/glossary/zh-cn.md"
  if (locale === "zht") return ".openchat/glossary/zh-tw.md"
  return `.openchat/glossary/${locale}.md`
}
...
Options:
  -c, --concurrency <count>  Maximum parallel OpenCode runs for 'all' (default: 4)
      --model <provider/id>  OpenCode model (default: openchat/gpt-5.5)
...
async function translate(
  ...
  env.OPENCHAT_DISABLE_PROJECT_CONFIG = "1"
  env.OPENCHAT_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["openchat", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OPENCHAT_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["openchat", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.OPENCHAT_CONFIG
  delete env.OPENCHAT_CONFIG_DIR
  delete env.OPENCHAT_CONFIG_CONTENT
  delete env.OPENCHAT_PERMISSION
  delete env.OPENCHAT_AUTO_SHARE
  return env
}
