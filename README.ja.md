...
brew install anomalyco/tap/openchat # macOS と Linux（推奨。常に最新）
...
nix run nixpkgs#openchat           # または github:anomalyco/openchat で最新 dev ブランチ
```
...
OpenCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/openchat/releases) から直接ダウンロードするか、[openchat.ai/download](https://openchat.ai/download) を利用してください。
...
OpenCode の設定については [**ドキュメント**](https://openchat.ai/docs) を参照してください。
...
OpenCode に関連するプロジェクトで、名前に "openchat"（例: "openchat-dashboard" や "openchat-mobile"）を含める場合は、そのプロジェクトが OpenCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。
