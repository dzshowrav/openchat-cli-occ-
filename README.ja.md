...
# パッケージマネージャー
npm i -g openchat-ai@latest        # bun/pnpm/yarn でもOK
scoop install openchat             # Windows
choco install openchat             # Windows
brew install dzshowrav/tap/openchat # macOS と Linux（推奨。常に最新）
brew install openchat              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S openchat            # Arch Linux (Stable)
paru -S openchat-bin               # Arch Linux (Latest from AUR)
mise use -g openchat               # どのOSでも
nix run nixpkgs#openchat           # または github:dzshowrav/openchat で最新 dev ブランチ
```
...
OpenChat はデスクトップアプリとしても利用できます。[releases page](https://github.com/dzshowrav/openchat/releases) から直接ダウンロードするか、[openchat.ai/download](https://openchat.ai/download) を利用してください。
...
1. `$OPENCHAT_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.openchat/bin` - デフォルトのフォールバック
...
OpenChat には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://openchat.ai/docs/agents) の詳細はこちら。

### ドキュメント

OpenChat の設定については [**ドキュメント**](https://openchat.ai/docs) を参照してください。

### コントリビュート

OpenChat に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### OpenChat の上に構築する

OpenChat に関連するプロジェクトで、名前に "openchat"（例: "openchat-dashboard" や "openchat-mobile"）を含める場合は、そのプロジェクトが OpenChat チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/openchat) | [X.com](https://x.com/openchat)
