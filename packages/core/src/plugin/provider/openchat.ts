...
import type { IntegrationOAuthMethodRegistration } from "@openchat-ai/plugin/v2/effect/integration"
import { define } from "@openchat-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@openchat-ai/sdk/v2/types"
...
const defaultServer = "https://console.openchat.ai"
const clientID = "openchat-cli"
...
export const OpenchatPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "openchat",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpenchatPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("openchat")
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("openchat", (integration) => {
        integration.name = "OpenCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "openchat", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("openchat")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("openchat")
      ...
      const item = catalog.provider.get(ProviderV2.ID.openchat)
      if (!item) return
      const hasKey = Boolean(process.env.OPENCHAT_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("openchat")),
