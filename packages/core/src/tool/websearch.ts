...
export class ConfigService extends Context.Service<ConfigService, Config>()("@openchat/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OPENCHAT_WEBSEARCH_PROVIDER === "exa" || process.env.OPENCHAT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OPENCHAT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("OPENCHAT_EXPERIMENTAL") || truthy("OPENCHAT_ENABLE_EXA") || truthy("OPENCHAT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OPENCHAT_ENABLE_PARALLEL") || truthy("OPENCHAT_EXPERIMENTAL_PARALLEL"),
