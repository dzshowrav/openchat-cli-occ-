...
import { GoogleVertexPlugin } from "@openchat-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.openchat, (provider) => {
          provider.api = {
            ...
            url: "https://openchat.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.openchat))
