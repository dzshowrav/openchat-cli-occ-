...
import { define } from "@openchat-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@openchat-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "openchat.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "openchat.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
