...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@openchat/acme")).toBe("@openchat/acme")
    expect(Npm.sanitize("@openchat/acme@1.0.0")).toBe("@openchat/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/openchat/acme.git"
    const expected = win ? "acme@git+https_//github.com/openchat/acme.git" : spec
