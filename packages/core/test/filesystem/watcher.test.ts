...
import { Watcher } from "@openchat-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    OPENCHAT_EXPERIMENTAL_FILEWATCHER: "true",
    OPENCHAT_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@openchat.test`.cwd(tmp.path).quiet()
