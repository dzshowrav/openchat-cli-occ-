...
import { createOpenchatClient } from "@openchat-ai/sdk/v2/client"
import { ServerAuth } from "@openchat-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createOpenchatClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@openchat/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createOpenchatClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createOpenchatClient({ baseUrl: connection.url, headers: connection.headers })
    })
