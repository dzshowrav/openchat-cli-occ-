...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve openchat route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "openchat")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "openchat")).toBe("openai")
    expect(statProvider("big-pickle", "", "openchat")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "openchat"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep openchat as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "openchat"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "openchat"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep openchat or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "openchat"), country: "US" })).toMatchObject([
