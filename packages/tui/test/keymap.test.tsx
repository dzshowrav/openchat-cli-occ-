...
import { getOpenchatModeStack, OPENCHAT_BASE_MODE, OpenchatKeymapProvider, registerOpenchatKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOpenchatKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when openchat mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOpenchatModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOpenchatModeStack(keymap).push("autocomplete")
