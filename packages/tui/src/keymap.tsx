...
export const OPENCHAT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENCHAT_MODE_KEY = "openchat.mode"

export const OpenchatKeymapProvider = KeymapProvider
export const useOpenchatKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OpenchatModeStack = ReturnType<typeof createOpenchatModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OpenchatModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOpenchatModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENCHAT_MODE_KEY, OPENCHAT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENCHAT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENCHAT_MODE_KEY, stack.at(-1)?.mode ?? OPENCHAT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENCHAT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENCHAT_MODE_KEY, undefined)
...
export function useOpenchatModeStack() {
  return getOpenchatModeStack(useOpenchatKeymap())
}

export function getOpenchatModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Openchat mode stack is not registered for this keymap")
  return value
}
...
export function registerOpenchatKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOpenchatModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useOpenchatKeymap()
