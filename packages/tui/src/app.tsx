...
import { registerOpenchatSpinner } from "./component/register-spinner"
...
import {
  ...
  OPENCHAT_BASE_MODE,
  OpenchatKeymapProvider,
  registerOpenchatKeymap,
  useBindings,
  useOpenchatKeymap,
} from "./keymap"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.OPENCHAT_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerOpenchatKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.OPENCHAT_ROUTE ? JSON.parse(process.env.OPENCHAT_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.OPENCHAT_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <OpenchatKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.OPENCHAT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.OPENCHAT_DISABLE_TERMINAL_TITLE) return
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.OPENCHAT_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://openchat.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.OPENCHAT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.OPENCHAT_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
