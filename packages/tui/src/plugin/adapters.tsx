import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@openchat-ai/plugin/tui"
...
import type { useOpenchatKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOpenchatKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOpenchatModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOpenchatModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
