...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@openchat-ai/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__openchat_custom_provider__"
    ...
    ...pipe(
      ...
      map((provider) => ({
        ...
        description: {
          ...
          "openchat-go": "Low cost subscription for everyone",
        }[provider.id],
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in openchat.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
        ...
        ({
          openchat: (
            <box gap={1}>
              ...
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://openchat.ai/zen</span> to get a key
              </text>
            </box>
          ),
          "openchat-go": (
            <box gap={1}>
              ...
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://openchat.ai/go</span> and enable OpenCode Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in openchat.json to use it.`,
          })
          dialog.clear()
          return
        }
