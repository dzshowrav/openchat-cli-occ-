...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("openchat-find")
  api.delete("openchat-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("openchat-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("openchat-find", new Highlight(...rest))
    return true
  }
