...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\openchat\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `openchat --log-level DEBUG`.

---

## Storage

openchat stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop runs a local OpenCode server (the `openchat-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (or `~/.config/openchat/openchat.json`)
- **macOS/Linux** (older installs): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Global plugins**
  ...
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\openchat\plugins`
...
- **Linux**: delete `~/.cache/openchat` (or run `rm -rf ~/.cache/openchat`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\openchat`
...
If your `openchat.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `OPENCHAT_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `OPENCHAT_PORT` (or pick a free port) and restart.
...
- `openchat.global.dat` and `openchat.workspace.*.dat` (UI state like recent servers/projects)
...
3. Ensure you have the latest version with `openchat upgrade`
...
To figure out what models you have access to, run `openchat models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\openchat`
...
If you encounter API call errors, this may be due to outdated provider packages. openchat dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\openchat`

2. Restart openchat to reinstall the latest provider packages

This will force openchat to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
openchat will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
