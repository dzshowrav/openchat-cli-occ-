...
デフォルトでは、OpenCode は独自の `openchat` テーマを使用します。
...
2. **ユーザー設定ディレクトリ** - `~/.config/openchat/themes/*.json` または `$XDG_CONFIG_HOME/openchat/themes/*.json` で定義されます
3. **プロジェクトのルートディレクトリ** - `<project-root>/.openchat/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.openchat/themes/*.json` で定義
...
1. **組み込みテーマ** - これらはバイナリに埋め込まれています
2. **ユーザー設定ディレクトリ** - `~/.config/openchat/themes/*.json` または `$XDG_CONFIG_HOME/openchat/themes/*.json` で定義されます
3. **プロジェクトのルート ディレクトリ** - `<project-root>/.openchat/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.openchat/themes/*.json` で定義
