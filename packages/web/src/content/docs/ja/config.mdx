...
1. **リモート設定** (`.well-known/openchat` から) - 組織のデフォルト
2. **グローバル設定** (`~/.config/openchat/openchat.json`) - ユーザー設定
3. **カスタム設定** (`OPENCHAT_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `openchat.json`) - プロジェクト固有の設定
5. **`.openchat` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`OPENCHAT_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド

つまり、プロジェクト設定はグローバルのデフォルトをオーバーライドでき、グローバル設定はリモート組織のデフォルトをオーバーライドできます。

:::note
`.openchat` および `~/.config/openchat` ディレクトリでは、サブディレクトリに **複数名** が使用されています: `agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/`、および `themes/`。下位互換性のために、単数形の名前 (`agent/` など) もサポートされています。
:::
...
組織は、`.well-known/openchat` エンドポイント経由でデフォルト設定を提供できます。これは、それをサポートするプロバイダーで認証するときに自動的に取得されます。
...
グローバル OpenCode 設定を `~/.config/openchat/openchat.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。

TUI 固有の設定には、`~/.config/openchat/tui.json` を使用します。
...
プロジェクトのルートに `openchat.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。
...
`OPENCHAT_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`OPENCHAT_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
環境変数。このディレクトリでは、エージェント、コマンド、
モードとプラグインは標準の `.openchat` ディレクトリと同様であり、次のようにする必要があります。
同じ構造に従います。
...
カスタムディレクトリはグローバル config ディレクトリと `.openchat` ディレクトリの後にロードされるため、それらの設定を**オーバーライド**できます。

---

## スキーマ

サーバー/ランタイム構成スキーマは [**`openchat.ai/config.json`**](https://openchat.ai/config.json) で定義されています。

TUI 設定は [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json) を使用します。
...
カスタム TUI 設定ファイルを指定するには、`OPENCHAT_TUI_CONFIG` を使用します。

`openchat.json` 内の従来の `theme`、`keybinds`、および `tui` キーは非推奨となり、可能であれば自動的に移行されます。
...
`openchat serve` オプションを使用して、`openchat web` および `server` コマンドのサーバー設定を構成できます。
...
- `mdnsDomain` - mDNS サービスのカスタムドメイン名。デフォルトは `openchat.local` です。同じネットワーク上で複数のインスタンスを実行する場合に便利です。
...
`~/.config/openchat/agents/` または `.openchat/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
この設定は、TUI、CLI (`openchat run`)、デスクトップアプリ、および GitHub Action のすべてのインターフェイスに適用されます。
...
`~/.config/openchat/commands/` または `.openchat/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
デフォルトでは、openchat は明示的な承認を必要とせずに **すべての操作を許可**します。これは、`permission` オプションを使用して変更できます。
...
プラグインファイルを `.openchat/plugins/` または `~/.config/openchat/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
