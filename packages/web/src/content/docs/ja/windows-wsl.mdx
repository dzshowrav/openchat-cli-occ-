...
:::
:::caution
`--hostname 0.0.0.0` を使う場合は、`OPENCHAT_SERVER_PASSWORD` を設定してサーバーを保護してください。
...
1. **PowerShell ではなく WSL ターミナルで `openchat web` を実行する**
...
WSL から `openchat web` を実行すると、適切なファイルシステムアクセスとターミナル統合を維持したまま、Windows ブラウザから利用できます。
...
- OpenCode の設定とセッションは WSL 環境内の `~/.local/share/openchat/` に保存されます
