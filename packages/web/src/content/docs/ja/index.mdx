...
![openchatテーマ](../../../assets/lander/screenshot.png)を使用したOpenCode TUI
...
- **macOS および Linux での Homebrew の使用**
  ...
  > 最新のリリースには OpenCode タップを使用することをお勧めします。公式の `brew install openchat` 式は Homebrew チームによって維持されており、更新頻度は低くなります。
...
[Releases](https://github.com/anomalyco/openchat/releases).
...
1. TUI で `/connect` コマンドを実行し、openchat を選択して、[openchat.ai/auth](https://openchat.ai/auth) で認証します。
...
これは [OpenCode を使用した会話 ](https://openchat.ai/s/4XP1fce5) の例です。
