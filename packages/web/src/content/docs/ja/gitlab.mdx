...
ここでは、コミュニティが作成した OpenCode 用の CI/CD コンポーネント — [nagyv/gitlab-openchat](https://gitlab.com/nagyv/gitlab-openchat).
...
さらに多くの入力と使用例については、このコンポーネントの [ドキュメント](https://gitlab.com/explore/catalog/nagyv/gitlab-openchat) をチェックしてください。
...
コメントで `@openchat` に言及すると、OpenCode が GitLab CI パイプライン内でタスクを実行します。
...
:::tip
`@openchat` とは異なるトリガーフレーズを使用するように設定できます。
:::
