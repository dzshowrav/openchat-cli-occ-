...
`openchat.json` 設定ファイルでエージェントを構成します。
...
- グローバル: `~/.config/openchat/agents/`
- プロジェクトごと: `.openchat/agents/`
...
OpenCode 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[OpenCode Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `openchat/gpt-5.1-codex` を使用します。
...
:::tip
`openchat models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/anomalyco/openchat) を送信します。
:::
