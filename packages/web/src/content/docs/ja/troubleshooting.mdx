...
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.local\share\openchat\log` を貼り付けます

ログファイルにはタイムスタンプ付きの名前が付けられ (例: `2025-01-09T123456.log`)、最新の 10 個のログファイルが保存されます。

`--log-level` コマンドラインオプションを使用してログレベルを設定すると、より詳細なデバッグ情報を取得できます。たとえば、`openchat --log-level DEBUG`。
...
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.local\share\openchat` を貼り付けます
...
OpenCode Desktop は、ローカル OpenCode サーバー (`openchat-cli` サイドカー) をバックグラウンドで実行します。ほとんどの問題は、誤動作しているプラグイン、破損したキャッシュ、または不正なサーバー設定によって発生します。
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (または `~/.config/openchat/openchat.json`)
- **macOS/Linux** (古いインストール): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\openchat\openchat.jsonc` を貼り付けます
...
- **グローバルプラグイン**
  ...
  - **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\openchat\plugins` を貼り付けます
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/openchat` を貼り付け
- **Linux**: `~/.cache/openchat` を削除します (または `rm -rf ~/.cache/openchat` を実行します)。
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.cache\openchat` を貼り付けます
...
`openchat.json(c)` に `server` セクションが含まれている場合は、それを一時的に削除し、デスクトップアプリを再起動します。

#### 環境変数を確認する

環境に `OPENCHAT_PORT` が設定されている場合、デスクトップアプリはローカルサーバーにそのポートを使用しようとします。

- `OPENCHAT_PORT` の設定を解除して (または空きポートを選択して)、再起動します。
...
- `openchat.settings.dat` (デスクトップのデフォルトサーバー URL)
- `openchat.global.dat` および `openchat.workspace.*.dat` (最近のサーバー/プロジェクトなどの UI 状態)
...
3. `openchat upgrade` を含む最新バージョンを使用していることを確認してください
...
どのモデルにアクセスできるかを確認するには、`openchat models` を実行します。
...
2. 問題が解決しない場合は、保存されている構成をクリアしてみてください。
   ...
   Windows では、`WIN+R` を押して、`%USERPROFILE%\.local\share\openchat` を削除します。
...
1. プロバイダーパッケージのキャッシュをクリアします。
   ...
   Windows では、`WIN+R` を押して、`%USERPROFILE%\.cache\openchat` を削除します。
