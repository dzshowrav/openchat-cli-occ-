...
- `.openchat/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/openchat/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/openchat/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/openchat/openchat.json`)
2. プロジェクト設定 (`openchat.json`)
3. グローバルプラグインディレクトリ (`~/.config/openchat/plugins/`)
4. プロジェクトプラグインディレクトリ (`.openchat/plugins/`)
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://openchat.ai/docs/sdk) を参照してください。
