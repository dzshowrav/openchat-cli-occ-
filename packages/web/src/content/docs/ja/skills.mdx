...
- プロジェクト設定: `.openchat/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/openchat/skills/<name>/SKILL.md`
...
途中で、一致する `skills/*/SKILL.md` を `.openchat/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/openchat/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.openchat/skills/git-release/SKILL.md` を作成します。
...
`openchat.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
