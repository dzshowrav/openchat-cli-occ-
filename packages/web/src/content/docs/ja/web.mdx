...
:::caution
`OPENCHAT_SERVER_PASSWORD` が設定されていない場合、サーバーは保護されません。これはローカルで使用する場合には問題ありませんが、ネットワークアクセス用に設定する必要があります。
:::
:::tip[Windows ユーザー]
最高のエクスペリエンスを得るには、PowerShell ではなく [WSL](/docs/windows-wsl) から `openchat web` を実行します。これにより、ファイルシステムへの適切なアクセスとターミナルの統合が保証されます。
:::
...
これにより、ホスト名が自動的に `0.0.0.0` に設定され、サーバーが `openchat.local` としてアドバタイズされます。
...
アクセスを保護するには、`OPENCHAT_SERVER_PASSWORD` 環境変数を使用してパスワードを設定します。

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

ユーザー名のデフォルトは `openchat` ですが、`OPENCHAT_SERVER_USERNAME` で変更できます。
...
`openchat.json` 設定ファイルでサーバー設定を構成することもできます。
