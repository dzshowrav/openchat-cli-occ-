...
`~/.local/share/openchat/auth.json`で。
...
1. TUI で `/connect` コマンドを実行し、`OpenCode Zen` を選択して、[openchat.ai/zen](https://openchat.ai/zen) にアクセスします。
...
1. TUI で `/connect` コマンドを実行し、`OpenCode Go` を選択して、[openchat.ai/zen](https://openchat.ai/zen) にアクセスします。
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   openchat の実行中に次の環境変数のいずれかを設定します。
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`openchat.json` を使用します。
...
openchat は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://atomic.chat) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `http://127.0.0.1:1337/v1`）。
...
`openchat.json` ファイル。セッション共有を無効にすることもお勧めします。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) プラグインを使用して、各 OpenCode 会話を Helicone のセッションとして自動的に記録します。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように openchat を構成できます。
...
LM Studio を通じてローカルモデルを使用するように openchat を構成できます。
...
Ollama を通じてローカルモデルを使用するように openchat を構成できます。

:::tip
Ollama は OpenCode 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/openchat)」を参照してください。
:::
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を Openchat で使用するには:
...
4. プロジェクトディレクトリで `openchat.json` ファイルを作成または更新します。
...
1. **認証設定を確認します**: `openchat auth list` を実行して、資格情報が正しいかどうかを確認します。
