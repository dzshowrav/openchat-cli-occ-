...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "openchat"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/openchat`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/openchat/openchat.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCHAT_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`openchat.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.openchat`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`OPENCHAT_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.openchat` و`~/.config/openchat` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/openchat`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات OpenCode العامة في `~/.config/openchat/openchat.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `openchat.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `OPENCHAT_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `OPENCHAT_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.openchat`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.openchat`، لذلك **يمكنه تجاوز** إعداداتها.

---

## المخطط

يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`openchat.ai/config.json`**](https://openchat.ai/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `openchat serve` و`openchat web` عبر الخيار `server`.
...
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `openchat.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/openchat/agents/` أو `.openchat/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`openchat run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/openchat/commands/` أو `.openchat/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
افتراضيًا يسمح openchat **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
ضع ملفات الإضافات في `.openchat/plugins/` أو `~/.config/openchat/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
