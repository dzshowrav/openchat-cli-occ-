...
:::note
هذه الأداة متاحة فقط عند ضبط `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (أو `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `OPENCHAT_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
