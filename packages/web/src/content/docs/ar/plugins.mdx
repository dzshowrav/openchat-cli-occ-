...
- `.openchat/plugins/` - إضافات على مستوى المشروع
- `~/.config/openchat/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/openchat/node_modules/`.
...
1. إعدادات عامة (`~/.config/openchat/openchat.json`)
2. إعدادات المشروع (`openchat.json`)
3. دليل الإضافات العام (`~/.config/openchat/plugins/`)
4. دليل إضافات المشروع (`.openchat/plugins/`)
...
- `client`: عميل SDK لـ openchat للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع openchat.
...
امنع openchat من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى openchat:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ openchat استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ openchat إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://openchat.ai/docs/sdk) للتفاصيل.
