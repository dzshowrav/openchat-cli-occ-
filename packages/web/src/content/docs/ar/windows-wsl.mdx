...
:::caution
عند استخدام `--hostname 0.0.0.0`، اضبط `OPENCHAT_SERVER_PASSWORD` لحماية الخادم.
...
1. **شغّل `openchat web` من terminal داخل WSL** بدلًا من PowerShell:
...
تشغيل `openchat web` من WSL يضمن وصولًا صحيحًا لنظام الملفات وتكاملًا أفضل مع terminal، مع بقائه متاحًا من متصفح Windows.
...
- إعدادات OpenCode وجلساته تُخزَّن داخل بيئة WSL في `~/.local/share/openchat/`
