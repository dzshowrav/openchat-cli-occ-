...
في `~/.local/share/openchat/auth.json`.
...
1. شغّل الأمر `/connect` في TUI، واختر openchat، ثم انتقل إلى [openchat.ai/auth](https://openchat.ai/auth).
...
1. شغّل الأمر `/connect` في TUI، واختر `OpenCode Go`، ثم انتقل إلى [openchat.ai/auth](https://openchat.ai/zen).
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل openchat:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `openchat.json`:
...
يمكنك تكوين openchat لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل openchat بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات openchat.
...
تخزين الرمز في مساحة تخزين مصادقة openchat.
:::
...
`openchat.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع openchat:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر openchat، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) لتسجيل كل محادثة OpenCode تلقائيا كجلسة في Helicone.
...
يمكنك ضبط openchat لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط openchat لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط openchat لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/openchat) للتفاصيل.
:::
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات openchat.
...
5. يمكنك أيضا تخصيصها عبر إعدادات openchat. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Openchat:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات openchat. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع openchat. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `openchat.json` في دليل مشروعك:
...
1. **تحقق من إعداد auth**: شغّل `openchat auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات openchat وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات openchat.
