...
افتراضيًا، يستخدم OpenCode سمتنا `openchat`.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openchat/themes/*.json` أو `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openchat/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openchat/themes/*.json`
