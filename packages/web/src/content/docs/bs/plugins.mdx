...
- `.openchat/plugins/` - Dodaci na nivou projekta
- `~/.config/openchat/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openchat/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openchat/openchat.json`)
2. Konfiguracija projekta (`openchat.json`)
3. Globalni direktorij dodataka (`~/.config/openchat/plugins/`)
4. Direktorij dodataka projekta (`.openchat/plugins/`)
...
- `client`: Openchat SDK klijent za interakciju sa AI.
...
Spriječite openchat da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openchat:
...
Pomoćnik `tool` kreira prilagođeni alat koji openchat može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openchat zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://openchat.ai/docs/sdk) za detalje.
