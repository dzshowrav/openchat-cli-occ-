...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openchat\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openchat --log-level DEBUG`.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop pokreće lokalni OpenCode server (`openchat-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (ili `~/.config/openchat/openchat.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Globalni dodaci**
  ...
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openchat\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openchat`
- **Linux**: obrišite `~/.cache/openchat` (ili pokrenite `rm -rf ~/.cache/openchat`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openchat`
...
Ako vaš `openchat.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENCHAT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENCHAT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
- `openchat.settings.dat` (URL zadanog servera za desktop)
- `openchat.global.dat` i `openchat.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `openchat upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `openchat models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openchat`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openchat`
