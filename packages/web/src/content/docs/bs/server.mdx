---
title: Server
description: Komunicirajte s openchat serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openchat serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openchat klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openchat.local` |
...
Postavite `OPENCHAT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openchat`, ili postavite `OPENCHAT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openchat serve` i za `openchat web`.
...
Kada pokrenete `openchat`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openchat server za programsku interakciju sa openchat-om.
:::

Ova arhitektura omogucava openchat podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openchat serve` da startate standalone server. Ako je openchat TUI vec pokrenut, `openchat serve` ce pokrenuti novi server.
...
openchat server izlaže sljedece API-je.
