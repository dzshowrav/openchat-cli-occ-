...
1. **Udaljena konfiguracija** (od `.well-known/openchat`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/openchat/openchat.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCHAT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`openchat.json` u projektu) - postavke specifične za projekat
5. **`.openchat` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENCHAT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.openchat` i `~/.config/openchat` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/openchat` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/openchat/openchat.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openchat/tui.json`.
...
Dodajte `openchat.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENCHAT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCHAT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openchat` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openchat`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`openchat.ai/config.json`**](https://openchat.ai/config.json).

TUI konfiguracija koristi [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json).
...
Koristite `OPENCHAT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `openchat.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `openchat serve` i `openchat web` putem opcije `server`.
...
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `openchat.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openchat/agents/` ili `.openchat/agents/`. [Saznajte više ovdje](/docs/agents).
...
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`openchat run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openchat/commands/` ili `.openchat/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.openchat/plugins/` ili `~/.config/openchat/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
