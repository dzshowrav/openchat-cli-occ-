...
:::caution
Ako `OPENCHAT_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::

:::tip[Windows korisnici]
Za najbolje iskustvo, pokrenite `openchat web` iz [WSL](/docs/windows-wsl) umjesto PowerShell-a. Ovo osigurava pravilan pristup sistemu datoteka i integraciju terminala.
:::
...
Ovo automatski postavlja ime hosta na `0.0.0.0` i oglašava server kao `openchat.local`.
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `OPENCHAT_SERVER_PASSWORD`:

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

Korisničko ime podrazumevano je `openchat`, ali se može promeniti sa `OPENCHAT_SERVER_USERNAME`.
...
Također možete konfigurirati postavke servera u svom `openchat.json` konfiguracijskom fajlu:
