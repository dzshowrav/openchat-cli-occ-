...
u `~/.local/share/openchat/auth.json`.
...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite openchat i idite na [openchat.ai/auth](https://openchat.ai/auth).
...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenCode Go` i idite na [openchat.ai/auth](https://openchat.ai/zen).
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openchat:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openchat.json`:
...
Možete konfigurirati openchat za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openchat ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openchat konfiguraciju.
...
da pohrani token u openchat auth memoriju.
:::
...
`openchat.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `openchat.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openchat:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openchat.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openchata, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) da automatski prijavite svaki OpenCode razgovor kao sesiju u Helicone-u.
...
Možete konfigurirati openchat za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openchat za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openchat za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openchat) za detalje.
:::
...
Također možete dodati dodatne modele putem vaše openchat konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openchat konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Openchatom:
...
Također možete prilagoditi modele kroz svoju openchat konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openchat-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openchat.json` datoteku u direktoriju projekta:
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openchat auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openchat konfiguraciji.
