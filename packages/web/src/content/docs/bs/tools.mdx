...
:::note
Ovaj alat je dostupan samo kada je `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `OPENCHAT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
