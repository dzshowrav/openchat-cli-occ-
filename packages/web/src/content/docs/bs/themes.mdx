...
Po defaultu, OpenCode koristi nasu `openchat` temu.
...
2. **Korisnicki config direktorij** - `~/.config/openchat/themes/*.json` ili `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openchat/themes/*.json`
4. **Trenutni radni direktorij** - `./.openchat/themes/*.json`
