...
OpenCode se integriše sa vašim GitHub tokovom rada. Spomenite `/openchat` ili `/oc` u svom komentaru i OpenCode će izvršiti zadatke unutar vašeg GitHub Actions runnera.
...
1. **Instalirajte GitHub aplikaciju**
   Idite na [**github.com/apps/openchatgent**](https://github.com/apps/openchatgent). Uvjerite se da je instaliran na ciljnom spremištu.
2. **Dodajte radni tok**
   Dodajte sljedeći fajl toka posla u `.github/workflows/openchat.yml` u svoj repo. Obavezno postavite odgovarajuće `model` i potrebne API ključeve u `env`.
...
|----------------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `issue_comment` | Komentirajte problem ili PR | Navedite `/openchat` ili `/oc` u svom komentaru. OpenCode čita kontekst i može kreirati grane, otvarati PR-ove ili odgovarati. |
| `pull_request_review_comment` | Komentirajte određene linije koda u PR-u | Navedite `/openchat` ili `/oc` dok pregledavate kod. OpenCode prima putanju datoteke, brojeve redova i kontekst razlike. |
