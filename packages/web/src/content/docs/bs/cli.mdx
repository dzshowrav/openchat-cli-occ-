...
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCHAT_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCHAT_SERVER_USERNAME` ili `openchat`) |
...
OpenCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `openchat auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/openchat/auth.json`.
...
Također možete priključiti pokrenutu `openchat serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `OPENCHAT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `openchat`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `OPENCHAT_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `openchat`).
...
| Varijabla                             | Tip     | Opis                                                               |
| ------------------------------------- | ------- | ------------------------------------------------------------------ |
| `OPENCHAT_AUTO_SHARE`                 | boolean | Automatski dijeli sesije                                           |
| `OPENCHAT_GIT_BASH_PATH`              | string  | Putanja do Git Bash izvršne datoteke na Windows-u                  |
| `OPENCHAT_CONFIG`                     | string  | Putanja do konfiguracijskog fajla                                  |
| `OPENCHAT_TUI_CONFIG`                 | string  | Putanja do TUI konfiguracijskog fajla                              |
| `OPENCHAT_CONFIG_DIR`                 | string  | Putanja do konfiguracijskog direktorija                            |
| `OPENCHAT_CONFIG_CONTENT`             | string  | Inline json konfiguracijski sadržaj                                |
| `OPENCHAT_DISABLE_AUTOUPDATE`         | boolean | Onemogući automatske provjere ažuriranja                           |
| `OPENCHAT_DISABLE_PRUNE`              | boolean | Onemogući brisanje (pruning) starih podataka                       |
| `OPENCHAT_DISABLE_TERMINAL_TITLE`     | boolean | Onemogući automatsko ažuriranje naslova terminala                  |
| `OPENCHAT_PERMISSION`                 | string  | Inline json konfiguracija dozvola                                  |
| `OPENCHAT_DISABLE_DEFAULT_PLUGINS`    | boolean | Onemogući podrazumijevane dodatke (plugins)                        |
| `OPENCHAT_DISABLE_LSP_DOWNLOAD`       | boolean | Onemogući automatsko preuzimanje LSP servera                       |
| `OPENCHAT_ENABLE_EXPERIMENTAL_MODELS` | boolean | Omogući eksperimentalne modele                                     |
| `OPENCHAT_DISABLE_AUTOCOMPACT`        | boolean | Onemogući automatsko sažimanje konteksta                           |
| `OPENCHAT_DISABLE_CLAUDE_CODE`        | boolean | Onemogući čitanje iz `.claude` (prompt + vještine)                 |
| `OPENCHAT_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Onemogući čitanje `~/.claude/CLAUDE.md`                            |
| `OPENCHAT_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Onemogući učitavanje `.claude/skills`                              |
| `OPENCHAT_DISABLE_MODELS_FETCH`       | boolean | Onemogući dohvaćanje modela iz udaljenih izvora                    |
| `OPENCHAT_FAKE_VCS`                   | string  | Lažni VCS provajder za potrebe testiranja                          |
| `OPENCHAT_CLIENT`                     | string  | Identifikator klijenta (zadano na `cli`)                           |
| `OPENCHAT_ENABLE_EXA`                 | boolean | Omogući Exa alate za web pretraživanje                             |
| `OPENCHAT_SERVER_PASSWORD`            | string  | Omogući osnovnu autentifikaciju za `serve`/`web`                   |
| `OPENCHAT_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `openchat`) |
| `OPENCHAT_MODELS_URL`                 | string  | Prilagođeni URL za dohvaćanje konfiguracije modela                 |
...
| Varijabla                                       | Tip     | Opis                                                    |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `OPENCHAT_EXPERIMENTAL`                         | boolean | Omogući eksperimentalne funkcije pod zbirnom zastavicom |
| `OPENCHAT_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Omogući otkrivanje ikona                                |
| `OPENCHAT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Onemogući kopiranje pri odabiru u TUI                   |
| `OPENCHAT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Zadano vremensko ograničenje za bash naredbe u ms       |
| `OPENCHAT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksimalni izlazni tokeni za LLM odgovore               |
| `OPENCHAT_EXPERIMENTAL_FILEWATCHER`             | boolean | Omogući praćenje datoteka za cijeli direktorij          |
| `OPENCHAT_EXPERIMENTAL_OXFMT`                   | boolean | Omogući oxfmt formatter                                 |
| `OPENCHAT_EXPERIMENTAL_LSP_TOOL`                | boolean | Omogući eksperimentalni LSP alat                        |
| `OPENCHAT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Onemogući praćenje datoteka                             |
| `OPENCHAT_EXPERIMENTAL_EXA`                     | boolean | Omogući eksperimentalne Exa funkcije                    |
| `OPENCHAT_EXPERIMENTAL_LSP_TY`                  | boolean | Omogući TY LSP za python datoteke                       |
| `OPENCHAT_EXPERIMENTAL_PLAN_MODE`               | boolean | Omogući Plan mod                                        |
| `OPENCHAT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Omogući pozadinske zadatke subagenata                   |
| `OPENCHAT_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Omogući eksperimentalni sistem događaja                 |
| `OPENCHAT_EXPERIMENTAL_NATIVE_LLM`              | boolean | Omogući nativnu putanju LLM zahtjeva                    |
| `OPENCHAT_EXPERIMENTAL_PARALLEL`                | boolean | Omogući paralelno izvršavanje web pretrage              |
| `OPENCHAT_EXPERIMENTAL_SCOUT`                   | boolean | Omogući Scout subagenta                                 |
| `OPENCHAT_EXPERIMENTAL_WORKSPACES`              | boolean | Omogući podršku za radne prostore                       |
