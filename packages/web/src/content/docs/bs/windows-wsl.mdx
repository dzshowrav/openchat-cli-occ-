...
:::caution
Kada koristite `--hostname 0.0.0.0`, postavite `OPENCHAT_SERVER_PASSWORD` da zastitite server.
...
1. **Pokrenite `openchat web` u WSL terminalu** umjesto u PowerShell-u:
...
Pokretanje `openchat web` iz WSL-a osigurava ispravan pristup fajl sistemu i terminalsku integraciju, a i dalje je dostupno iz Windows browsera.
...
- Vase OpenCode konfiguracije i sesije cuvaju se u WSL okruzenju na `~/.local/share/openchat/`
