---
title: Servidor
description: Interactuar con el servidor openchat a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openchat serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openchat puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openchat.local` |
...
Configure `OPENCHAT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openchat`, o configure `OPENCHAT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openchat serve` como a `openchat web`.
...
Cuando ejecuta `openchat`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openchat para interactuar con openchat mediante programación.
:::

Esta arquitectura permite que openchat admita múltiples clientes y le permite interactuar con openchat mediante programación.

Puede ejecutar `openchat serve` para iniciar un servidor independiente. Si tienes el
openchat TUI ejecutándose, `openchat serve` iniciará un nuevo servidor.
...
El servidor openchat expone las siguientes API.
