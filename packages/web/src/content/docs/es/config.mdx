...
Por ejemplo, si su configuración global establece `theme: "openchat"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/openchat`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/openchat/openchat.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCHAT_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`openchat.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.openchat`** - agentes, comandos, complementos
6. **Configuración en línea** (`OPENCHAT_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.openchat` y `~/.config/openchat` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/openchat`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global OpenCode en `~/.config/openchat/openchat.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `openchat.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `OPENCHAT_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `OPENCHAT_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.openchat`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.openchat`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`openchat.ai/config.json`**](https://openchat.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `openchat serve` y `openchat web` a través de la opción `server`.
...
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `openchat.local`. Útil para ejecutar múltiples instancias en la misma red.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/openchat/agents/` o `.openchat/agents/`. [Más información aquí](/docs/agents).
...
Esta configuración se aplica en todas las interfaces: TUI, CLI (`openchat run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/openchat/commands/` o `.openchat/commands/`. [Más información aquí](/docs/commands).
...
De forma predeterminada, openchat **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Coloque los archivos de complemento en `.openchat/plugins/` o `~/.config/openchat/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
