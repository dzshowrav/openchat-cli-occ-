...
- `.openchat/plugins/` - Complementos a nivel de proyecto
- `~/.config/openchat/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openchat/node_modules/`.
...
1. Configuración global (`~/.config/openchat/openchat.json`)
2. Configuración del proyecto (`openchat.json`)
3. Directorio global de complementos (`~/.config/openchat/plugins/`)
4. Directorio de complementos del proyecto (`.openchat/plugins/`)
...
- `client`: Un cliente SDK openchat para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openchat.
...
Evite que openchat lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a openchat:
...
El ayudante `tool` crea una herramienta personalizada a la que openchat puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openchat junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://openchat.ai/docs/sdk) para obtener más detalles.
