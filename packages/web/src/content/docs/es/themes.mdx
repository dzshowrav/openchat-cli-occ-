...
De forma predeterminada, OpenCode usa nuestro propio tema `openchat`.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openchat/themes/*.json` o `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openchat/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openchat/themes/*.json`
