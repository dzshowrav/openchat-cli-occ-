...
:::caution
Cuando uses `--hostname 0.0.0.0`, define `OPENCHAT_SERVER_PASSWORD` para proteger el servidor.
...
1. **Ejecuta `openchat web` en la terminal de WSL** en lugar de PowerShell:
...
Ejecutar `openchat web` desde WSL garantiza acceso correcto al sistema de archivos e integración con la terminal, manteniéndolo accesible desde tu navegador de Windows.
...
- Tu configuración y tus sesiones de OpenCode se guardan dentro del entorno WSL en `~/.local/share/openchat/`
