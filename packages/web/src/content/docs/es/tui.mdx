...
Esto es independiente de `openchat.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
Utilice `OPENCHAT_TUI_CONFIG` para cargar una ruta de configuración de TUI personalizada.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `openchat.default`.
