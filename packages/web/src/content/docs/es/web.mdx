...
:::caution
Si no se establece `OPENCHAT_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::

:::tip[Windows Usuarios]
Para obtener la mejor experiencia, ejecute `openchat web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
Esto establece automáticamente el nombre de host en `0.0.0.0` y anuncia el servidor como `openchat.local`.
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `OPENCHAT_SERVER_PASSWORD`:

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

El nombre de usuario predeterminado es `openchat` pero se puede cambiar con `OPENCHAT_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `openchat.json`:
