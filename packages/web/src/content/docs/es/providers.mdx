...
1. Ejecute el comando `/connect` en TUI, seleccione openchat y diríjase a [openchat.ai/auth](https://openchat.ai/auth).
...
1. Ejecute el comando `/connect` en TUI, seleccione `OpenCode Go` y diríjase a [openchat.ai/auth](https://openchat.ai/zen).
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta openchat:
   ...
   Para una configuración persistente o específica del proyecto, utilice `openchat.json`:
...
Puedes configurar openchat para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que openchat funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración openchat.
...
para almacenar el token en el almacenamiento de autenticación openchat.
:::
...
Archivo `openchat.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `openchat.json`:
...
Para utilizar su suscripción GitHub Copilot con openchat:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta openchat.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de openchat, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) para registrar automáticamente cada conversación OpenCode como una sesión en Helicone.
...
Puede configurar openchat para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar openchat para usar modelos locales a través de LM Studio.
...
Puedes configurar openchat para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/openchat) para obtener más detalles.
:::
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración openchat.
...
5. También puedes personalizarlos a través de tu configuración openchat. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
También puede personalizar modelos a través de su configuración openchat. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con openchat. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `openchat.json` en el directorio de su proyecto:
...
1. **Verifique la configuración de autenticación**: Ejecute `openchat auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración openchat y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración openchat.
