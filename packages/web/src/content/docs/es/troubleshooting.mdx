...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openchat\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openchat --log-level DEBUG`.

---

## Almacenamiento

openchat almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop ejecuta un servidor OpenCode local (el sidecar `openchat-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (o `~/.config/openchat/openchat.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Complementos globales**
  ...
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openchat\plugins`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openchat`
- **Linux**: eliminar `~/.cache/openchat` (o ejecutar `rm -rf ~/.cache/openchat`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openchat`
...
Si su `openchat.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OPENCHAT_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OPENCHAT_PORT` (o elija un puerto libre) y reinicie.
...
- `openchat.settings.dat` (URL del servidor predeterminado de escritorio)
- `openchat.global.dat` y `openchat.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `openchat upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `openchat models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openchat`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openchat instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openchat`

2. Reinicie openchat para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openchat a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openchat detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
