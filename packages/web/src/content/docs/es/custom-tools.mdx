...
description: Cree herramientas que LLM pueda llamar en openchat.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openchat como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openchat/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openchat/tools/`.
...
```ts title=".openchat/tools/database.ts" {1}
...
```ts title=".openchat/tools/math.ts"
import { tool } from "@openchat-ai/plugin"
...
```ts title=".openchat/tools/bash.ts"
import { tool } from "@openchat-ai/plugin"
...
```ts title=".openchat/tools/project.ts" {8}
import { tool } from "@openchat-ai/plugin"
...
```python title=".openchat/tools/add.py"
import sys
...
```ts title=".openchat/tools/python-add.ts" {10}
import { tool } from "@openchat-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openchat/tools/add.py")
