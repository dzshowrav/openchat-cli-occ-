...
:::note
Esta herramienta solo está disponible cuando `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (o `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `OPENCHAT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).
