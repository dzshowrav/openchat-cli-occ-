...
`.openchat/commands/test.md` oluştur:
...
openchat işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

openchat [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/openchat/commands/`
- Proje başına: `.openchat/commands/`
...
openchat, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
