...
_Model Bağlam Protokolü_ veya MCP kullanarak openchat'a harici araçlar ekleyebilirsiniz. openchat hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openchat Config](https://openchat.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.

```jsonc title="openchat.jsonc" {6}
{
...
Kuruluşlar, `.well-known/openchat` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openchat, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openchat şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openchat, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openchat mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
```bash
openchat mcp auth my-oauth-server
```
...
```bash
openchat mcp logout my-oauth-server
```

`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openchat, belirteçleri `~/.local/share/openchat/mcp-auth.json`'de güvenli bir şekilde saklar.
...
openchat mcp debug my-oauth-server
```
...
MCP'leriniz yerleşik araçların yanı sıra openchat'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openchat yapılandırması aracılığıyla yönetebilirsiniz.
...
```json title="openchat.json" {11, 14-18}
{
  "$schema": "https://openchat.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
openchat mcp auth sentry
```

Bu, OAuth akışını tamamlamak ve openchat'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
