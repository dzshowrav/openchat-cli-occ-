...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `openchat.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `OPENCHAT_TUI_CONFIG` kullanın.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `openchat.default`.
