...
- `.openchat/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openchat/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openchat/node_modules/`'da önbelleğe alınır.
...
3. Global eklenti dizini (`~/.config/openchat/plugins/`)
4. Proje eklenti dizini (`.openchat/plugins/`)
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://openchat.ai/docs/sdk) bakın.
