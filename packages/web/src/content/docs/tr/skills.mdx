...
Ajan becerileri, openchat'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openchat su konumlari tarar:

- Proje konfigurasyonu: `.openchat/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openchat/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openchat, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openchat/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openchat/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openchat/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openchat, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openchat.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openchat.json` icinde):
