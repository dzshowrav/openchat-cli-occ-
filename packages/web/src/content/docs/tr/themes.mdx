...
openchat ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openchat kendi `openchat` temasını kullanır.
...
openchat birden fazla yerleşik temayla gelir.
...
- openchat'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openchat, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openchat/themes/*.json` veya `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openchat/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openchat/themes/*.json`
