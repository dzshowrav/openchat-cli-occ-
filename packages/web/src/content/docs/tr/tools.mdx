...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openchat yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openchat içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENCHAT_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openchat provider kullanılırken veya `OPENCHAT_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openchat başlatırken etkinleştirmek için:
