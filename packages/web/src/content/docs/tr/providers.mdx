---
title: Sağlayıcılar
description: openchat'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

openchat, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı openchat yapılandırmanızda yapılandırın.
...
`~/.local/share/openchat/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları openchat'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenCode Zen, openchat ekibi tarafından sağlanan modellerin bir listesidir.
openchat ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, openchat'u seçin ve [openchat.ai/auth](https://openchat.ai/auth)'ye gidin.
...
openchat'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenCode Go

OpenCode Go, openchat ile iyi çalıştığı test edilmiş ve doğrulanmış, openchat ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenCode Go`'yu seçin ve [openchat.ai/auth](https://openchat.ai/zen) adresine gidin.
...
Amazon Bedrock'u openchat ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   openchat'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `openchat.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi openchat'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
openchat'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   openchat'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Openchat yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in openchat auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
openchat, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. openchat'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`openchat.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`openchat.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi openchat'la kullanmak için:
...
Google Vertex AI'yi openchat ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları openchat çalıştırılırken ayarlayın.
...
Helicone'un openchat aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her openchat görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her openchat görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
openchat'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla openchat'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla openchat'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini openchat için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/openchat)'a bakın.
:::
...
Ollama Cloud'u openchat ile kullanmak için:
...
3. openchat'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: openchat'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenCode Zen, openchat ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. openchat API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   openchat yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca openchat yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Openchat ile kullanmak için:
...
Modelleri openchat yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı openchat ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
3. Sağlayıcı için API anahtarınızı girin.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in openchat.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Proje dizininizde `openchat.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları openchat'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `openchat auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için openchat yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, openchat yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
