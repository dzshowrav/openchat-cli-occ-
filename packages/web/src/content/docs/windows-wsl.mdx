...
:::caution
When using `--hostname 0.0.0.0`, set `OPENCHAT_SERVER_PASSWORD` to secure the server.
:::
...
1. **Run `openchat web` in the WSL terminal** rather than PowerShell:
...
Running `openchat web` from WSL ensures proper file system access and terminal integration while still being accessible from your Windows browser.
...
- Your OpenCode config and sessions are stored within the WSL environment at `~/.local/share/openchat/`
