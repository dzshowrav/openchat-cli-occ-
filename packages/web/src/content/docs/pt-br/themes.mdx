...
Com o openchat, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openchat usa nosso próprio tema `openchat`.
...
O openchat vem com vários temas embutidos.
...
- Querem que o openchat corresponda à aparência do seu terminal
...
O openchat suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openchat/themes/*.json` ou `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openchat/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openchat/themes/*.json`
