---
title: Provedores
description: Usando qualquer provedor LLM no openchat.
---

import config from "../../../../config.mjs"
export const console = config.console

openchat usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do openchat.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/openchat/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do openchat.
...
OpenCode Zen é uma lista de modelos fornecidos pela equipe do openchat que foram testados e verificados para funcionar bem com o openchat. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OpenCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OpenCode Zen` e acesse [openchat.ai/zen](https://openchat.ai/zen).
...
Funciona como qualquer outro provedor no openchat e é completamente opcional.

---

## OpenCode Go

OpenCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do openchat que foram
testados e verificados para funcionar bem com o openchat.

1. Execute o comando `/connect` no TUI, selecione `OpenCode Go` e acesse [openchat.ai/zen](https://openchat.ai/zen).
...
Para usar o Amazon Bedrock com o openchat:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o openchat:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY openchat

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile openchat

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX openchat
   ```
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `openchat.json`:

   ```json title="openchat.json"
   {
     "$schema": "https://openchat.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Usar sua assinatura Claude Pro/Max no openchat não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o openchat para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).

```json title="openchat.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o openchat funcione corretamente.
   :::
...
5. Defina o nome do seu recurso como uma variável de ambiente:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX openchat
   ```
...
5. Defina o nome do seu recurso como uma variável de ambiente:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX openchat
   ```
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do openchat.
...
armazenar o token no armazenamento de autenticação do openchat.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
openchat usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o openchat
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `openchat.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `openchat.json`:
...
Para usar sua assinatura do GitHub Copilot com o openchat:
...
Para usar o Google Vertex AI com o openchat:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o openchat.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id openchat
   ```
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do openchat, você sempre pode configurá-lo você mesmo.
...
```jsonc title="~/.config/openchat/openchat.jsonc"
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "helicone": {
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) para registrar automaticamente cada conversa do openchat como uma sessão no Helicone.

```bash
npm install -g openchat-helicone-session
```
...
```json title="openchat.json"
{
  "plugin": ["openchat-helicone-session"]
}
```

O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do openchat listada como uma sessão separada.
...
Você pode configurar o openchat para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)

```json title="openchat.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Você pode configurar o openchat para usar modelos locais através do LM Studio.

```json title="openchat.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "lmstudio": {
...
Você pode configurar o openchat para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o openchat. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/openchat) para detalhes.
:::

```json title="openchat.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "ollama": {
...
Para usar o Ollama Cloud com o openchat:
...
3. Copie a chave da API para uso no openchat.
...
6. **Importante**: Antes de usar modelos em nuvem no openchat, você deve puxar as informações do modelo localmente:
...
OpenCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do openchat. [Saiba mais](/docs/zen).
...
3. Insira sua chave da API do openchat.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do openchat.
...
5. Você também pode personalizá-los através da sua configuração do openchat. Aqui está um exemplo de especificar um provedor
...
3. Insira seu JSON da chave de serviço.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' openchat
   ```
...
4. Opcionalmente, defina o ID de implantação e o grupo de recursos:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group openchat
   ```
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o openchat:
...
Você também pode personalizar modelos através da sua configuração do openchat. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o openchat. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no openchat.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `openchat.json` no diretório do seu projeto:

   ```json title="openchat.json" ""myprovider"" {5-15}
   {
     "$schema": "https://openchat.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="openchat.json" {9,11,17-20}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "myprovider": {
...
Os campos `limit` permitem que o openchat entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `openchat auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do openchat e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do openchat.
