---
title: Plugins
description: Escreva seus próprios plugins para estender o openchat.
---

Plugins permitem que você estenda o openchat conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openchat.
...
- `.openchat/plugins/` - Plugins em nível de projeto
- `~/.config/openchat/plugins/` - Plugins globais
...
```json title="openchat.json"
{
  ...
  "plugin": ["openchat-helicone-session", "openchat-wakatime", "@my-org/custom-plugin"]
}
```
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openchat/node_modules/`.
...
1. Configuração global (`~/.config/openchat/openchat.json`)
2. Configuração do projeto (`openchat.json`)
3. Diretório de plugins global (`~/.config/openchat/plugins/`)
4. Diretório de plugins do projeto (`.openchat/plugins/`)
...
```json title=".openchat/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
O openchat executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.

```ts title=".openchat/plugins/my-plugin.ts"
...
```js title=".openchat/plugins/example.js"
...
- `client`: Um cliente SDK do openchat para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openchat.
...
```js title=".openchat/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openchat"'`
      }
    },
  }
}
...
:::note
Se você estiver usando o aplicativo desktop openchat, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openchat de ler arquivos `.env`:

```javascript title=".openchat/plugins/env-protection.js"
...
```javascript title=".openchat/plugins/inject-env.js"
...
Plugins também podem adicionar ferramentas personalizadas ao openchat:

```ts title=".openchat/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openchat-ai/plugin"
...
O helper `tool` cria uma ferramenta personalizada que o openchat pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openchat junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://openchat.ai/docs/sdk) para detalhes.
...
```ts title=".openchat/plugins/compaction.ts"
import type { Plugin } from "@openchat-ai/plugin"
...
```ts title=".openchat/plugins/custom-compaction.ts"
import type { Plugin } from "@openchat-ai/plugin"
