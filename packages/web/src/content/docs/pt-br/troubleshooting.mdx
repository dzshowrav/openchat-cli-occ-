...
Para depurar problemas com o openchat, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **macOS/Linux**: `~/.local/share/openchat/log/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\openchat\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `openchat --log-level DEBUG`.

---

## Armazenamento

O openchat armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\openchat`
...
O openchat Desktop executa um servidor openchat local (o sidecar `openchat-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (ou `~/.config/openchat/openchat.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
O openchat também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/openchat/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\openchat\plugins`
- **Plugins de projeto** (apenas se você usar configuração por projeto)
  - `<your-project>/.openchat/plugins/`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o openchat possa reconstruí-lo.

1. Saia completamente do openchat Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/openchat`
- **Linux**: exclua `~/.cache/openchat` (ou execute `rm -rf ~/.cache/openchat`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\openchat`

3. Reinicie o openchat Desktop.

---

### Corrigindo problemas de conexão com o servidor

O openchat Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `openchat.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `OPENCHAT_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `OPENCHAT_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o openchat Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do openchat.

---

### Notificações não aparecendo

O openchat Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o openchat nas configurações do seu sistema operacional, e
...
1. Saia do openchat Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo openchat Desktop):

- `openchat.settings.dat` (URL do servidor padrão do desktop)
- `openchat.global.dat` e `openchat.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o openchat:
...
1. **Relatar problemas no GitHub**
   ...
   [**github.com/anomalyco/openchat/issues**](https://github.com/anomalyco/openchat/issues)
...
2. **Junte-se ao nosso Discord**
   ...
   [**openchat.ai/discord**](https://openchat.ai/discord)
...
Para descobrir quais modelos você tem acesso, execute `openchat models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   ```bash
   rm -rf ~/.local/share/openchat
   ```

   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\openchat`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O openchat instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   ```bash
   rm -rf ~/.cache/openchat
   ```

   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\openchat`

2. Reinicie o openchat para reinstalar os pacotes de provedores mais recentes

Isso forçará o openchat a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O openchat detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
