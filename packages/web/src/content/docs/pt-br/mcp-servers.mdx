...
Você pode adicionar ferramentas externas ao openchat usando o _Model Context Protocol_, ou MCP. O openchat suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do openchat](https://openchat.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/openchat`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O openchat lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o openchat irá:
...
Se o servidor requer autenticação, o openchat solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `openchat mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o openchat armazenará os tokens de forma segura em `~/.local/share/openchat/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no openchat, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do openchat como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o openchat à sua conta do Sentry.
