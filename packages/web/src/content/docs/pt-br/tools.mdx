...
As ferramentas permitem que o LLM execute ações em sua base de código. O openchat vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no openchat.
...
:::note
Esta ferramenta está disponível apenas quando `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (ou `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor openchat ou quando a variável de ambiente `OPENCHAT_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o openchat:

```bash
OPENCHAT_ENABLE_EXA=1 openchat
```
