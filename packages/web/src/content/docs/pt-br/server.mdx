---
title: Servidor
description: Interaja com o servidor openchat via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openchat serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openchat pode usar.
...
```bash
openchat serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openchat.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
openchat serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCHAT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openchat`, ou defina `OPENCHAT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openchat serve` quanto ao `openchat web`.

```bash
OPENCHAT_SERVER_PASSWORD=your-password openchat serve
```
...
Quando você executa `openchat`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openchat para interagir com o openchat programaticamente.
:::

Essa arquitetura permite que o openchat suporte múltiplos clientes e permite que você interaja com o openchat programaticamente.

Você pode executar `openchat serve` para iniciar um servidor autônomo. Se você tiver o TUI do openchat em execução, `openchat serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openchat [IDE](/docs/ide).
...
O servidor openchat expõe as seguintes APIs.
