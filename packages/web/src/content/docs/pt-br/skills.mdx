...
As habilidades do agente permitem que o openchat descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openchat pesquisa nesses locais:

- Configuração do projeto: `.openchat/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openchat/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openchat sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openchat/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openchat/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openchat/skills/git-release/SKILL.md` assim:
...
O openchat lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openchat.json`:
...
**Para agentes embutidos** (em `openchat.json`):
