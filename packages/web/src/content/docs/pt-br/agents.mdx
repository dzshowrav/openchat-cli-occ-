...
Existem dois tipos de agentes no openchat; agentes primários e subagentes.
...
openchat vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Configure os agentes em seu arquivo de configuração `openchat.json`:
...
- Global: `~/.config/openchat/agents/`
- Por projeto: `.openchat/agents/`

```markdown title="~/.config/openchat/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o openchat usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openchat quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openchat usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `openchat/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="openchat.json" {3-6,9-12}
{
  "$schema": "https://openchat.ai/config.json",
  "tools": {
    "write": true,
...
```json title="openchat.json" {3-5,8-10}
{
  "$schema": "https://openchat.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `openchat models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/openchat).
:::
...
```markdown title="~/.config/openchat/agents/docs-writer.md"
---
...
```markdown title="~/.config/openchat/agents/security-auditor.md"
---
