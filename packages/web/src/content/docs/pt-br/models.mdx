...
O openchat usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

---

## Provedores

Os provedores mais populares são pré-carregados por padrão. Se você adicionou as credenciais para um provedor através do comando `/connect`, elas estarão disponíveis quando você iniciar o openchat.
...
Aqui estão vários modelos que funcionam bem com o openchat, em nenhuma ordem específica. (Esta não é uma lista exaustiva nem necessariamente atualizada):
...
Para definir um desses como o modelo padrão, você pode definir a chave `model` na sua configuração do openchat.

```json title="openchat.json" {3}
{
...
Aqui, o ID completo é `provider_id/model_id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `openchat/gpt-5.1-codex` para GPT 5.1 Codex.
...
```jsonc title="openchat.jsonc" {7-12,19-24}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="openchat.jsonc" {6-21}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "openchat": {
...
Muitos modelos suportam várias variantes com diferentes configurações. O openchat vem com variantes padrão integradas para provedores populares.

### Variantes integradas

O openchat vem com variantes padrão para muitos provedores:
...
```jsonc title="openchat.jsonc" {7-18}
{
  "$schema": "https://openchat.ai/config.json",
  "provider": {
    "openai": {
...
Quando o openchat é iniciado, ele verifica modelos na seguinte ordem de prioridade:

1. A flag de linha de comando `--model` ou `-m`. O formato é o mesmo que no arquivo de configuração: `provider_id/model_id`.

2. A lista de modelos na configuração do openchat.
