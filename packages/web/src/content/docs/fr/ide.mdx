...
OpenCode s'intègre à VS Code, Cursor ou tout IDE prenant en charge un terminal. Exécutez simplement `openchat` dans le terminal pour commencer.
...
3. Exécutez `openchat` - l'extension s'installe automatiquement
...
- Assurez-vous que vous exécutez `openchat` dans le terminal intégré.
