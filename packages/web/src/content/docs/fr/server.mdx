---
title: Serveur
description: Interagissez avec le serveur openchat via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openchat serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openchat peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openchat.local` |
...
Définissez `OPENCHAT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openchat`, ou définissez `OPENCHAT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openchat serve` et à `openchat web`.
...
Lorsque vous exécutez `openchat`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openchat pour interagir avec openchat par programmation.
:::

Cette architecture permet à openchat de prendre en charge plusieurs clients et vous permet d'interagir avec openchat par programme.

Vous pouvez exécuter `openchat serve` pour démarrer un serveur autonome. Si vous avez le
openchat TUI en cours d'exécution, `openchat serve` démarrera un nouveau serveur.
...
Le serveur openchat expose les API suivantes.
