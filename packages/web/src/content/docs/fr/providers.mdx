...
en `~/.local/share/openchat/auth.json`.
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez openchat et dirigez-vous vers [openchat.ai/auth](https://openchat.ai/auth).
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenCode Go` et rendez-vous sur [openchat.ai/auth](https://openchat.ai/zen).
...
Définissez l'une de ces variables d'environnement lors de l'exécution de openchat :
...
Pour une configuration spécifique au projet ou persistante, utilisez `openchat.json` :
...
Vous pouvez configurer openchat pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que openchat fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration openchat.
...
pour stocker le jeton dans le stockage d'authentification openchat.
:::
...
Fichier `openchat.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `openchat.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec openchat :
...
Définissez-les lors de l'exécution de openchat.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via openchat, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) pour enregistrer automatiquement chaque conversation OpenCode en tant que session dans Helicone.
...
Vous pouvez configurer openchat pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer openchat pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer openchat pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/openchat) pour plus de détails.
:::
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration openchat.
...
5. Vous pouvez également les personnaliser via votre configuration openchat. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Openchat :
...
Vous pouvez également personnaliser les modèles via votre configuration openchat. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec openchat. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `openchat.json` dans le répertoire de votre projet :
...
1. **Vérifiez la configuration de l'authentification** : exécutez `openchat auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration openchat et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration openchat.
