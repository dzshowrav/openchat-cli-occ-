...
Ici, nous utilisons un composant CI/CD créé par la communauté pour OpenCode — [nagyv/gitlab-openchat](https://gitlab.com/nagyv/gitlab-openchat).
...
Pour plus d'entrées et de cas d'utilisation [consultez la doc](https://gitlab.com/explore/catalog/nagyv/gitlab-openchat) pour ce composant.
...
Mentionnez `@openchat` dans un commentaire et OpenCode exécutera les tâches au sein de votre pipeline CI GitLab.
...
:::tip
Vous pouvez configurer pour utiliser une phrase de déclenchement différente de `@openchat`.
:::
