...
Par défaut, OpenCode utilise notre propre thème `openchat`.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openchat/themes/*.json` ou `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openchat/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openchat/themes/*.json`
