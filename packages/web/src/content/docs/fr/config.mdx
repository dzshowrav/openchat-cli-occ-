...
1. **Configuration distante** (à partir de `.well-known/openchat`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/openchat/openchat.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENCHAT_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`openchat.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.openchat`** - agents, commandes, plugins
6. **Configuration en ligne** (`OPENCHAT_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.openchat` et `~/.config/openchat` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/openchat`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale OpenCode dans `~/.config/openchat/openchat.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/openchat/tui.json`.
...
Ajoutez `openchat.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `OPENCHAT_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENCHAT_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.openchat`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.openchat`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`openchat.ai/config.json`**](https://openchat.ai/config.json).

La configuration TUI utilise [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json).
...
Utilisez `OPENCHAT_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `openchat.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `openchat serve` et `openchat web` via l'option `server`.
...
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `openchat.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/openchat/agents/` ou `.openchat/agents/`. [En savoir plus ici](/docs/agents).
...
Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`openchat run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/openchat/commands/` ou `.openchat/commands/`. [En savoir plus ici](/docs/commands).
...
Par défaut, openchat **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
Placez les fichiers du plugin dans `.openchat/plugins/` ou `~/.config/openchat/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
