...
:::caution
Si `OPENCHAT_SERVER_PASSWORD` n'est pas défini, le serveur ne sera pas sécurisé. C'est bien pour une utilisation locale mais doit être défini pour l'accès au réseau.
:::

:::tip[Windows Utilisateurs]
Pour une expérience optimale, exécutez `openchat web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Cela définit automatiquement le nom d'hôte sur `0.0.0.0` et annonce le serveur comme `openchat.local`.
...
Pour protéger l'accès, définissez un mot de passe à l'aide de la variable d'environnement `OPENCHAT_SERVER_PASSWORD` :

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

Le nom d'utilisateur par défaut est `openchat` mais peut être modifié avec `OPENCHAT_SERVER_USERNAME`.
...
Vous pouvez également configurer les paramètres du serveur dans votre fichier de configuration `openchat.json` :
