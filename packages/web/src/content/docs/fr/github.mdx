...
OpenCode s'intègre à votre flux de travail GitHub. Mentionnez `/openchat` ou `/oc` dans votre commentaire, et OpenCode exécutera des tâches dans votre runner GitHub Actions.
...
Rendez-vous sur [**github.com/apps/openchat-agent**](https://github.com/apps/openchat-agent). Assurez-vous qu'il est installé sur le dépôt cible.

2. **Ajouter le workflow**

Ajoutez le fichier de workflow suivant à `.github/workflows/openchat.yml` dans votre dépôt. Assurez-vous de définir les clés `model` appropriées et les clés API requises dans `env`.
...
| ----------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `issue_comment`               | Commentaire sur une issue ou une PR                        | Mentionnez `/openchat` ou `/oc` dans votre commentaire. OpenCode lit le contexte et peut créer des branches, ouvrir des PR ou répondre.                |
| `pull_request_review_comment` | Commentaire sur des lignes de code spécifiques dans une PR | Mentionnez `/openchat` ou `/oc` lors de la révision du code. OpenCode reçoit le chemin du fichier, les numéros de ligne et le contexte de comparaison. |
