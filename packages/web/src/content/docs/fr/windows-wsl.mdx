...
:::caution
Lorsque vous utilisez `--hostname 0.0.0.0`, définissez `OPENCHAT_SERVER_PASSWORD` pour sécuriser le serveur.
...
1. **Exécutez `openchat web` dans le terminal WSL** plutôt que dans PowerShell:
...
Lancer `openchat web` depuis WSL garantit un accès correct au système de fichiers et une bonne intégration terminal, tout en restant accessible depuis votre navigateur Windows.
...
- Votre configuration OpenCode et vos sessions sont stockées dans l'environnement WSL à `~/.local/share/openchat/`
