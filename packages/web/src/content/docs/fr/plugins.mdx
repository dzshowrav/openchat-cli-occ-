...
- `.openchat/plugins/` - Plugins au niveau du projet
- `~/.config/openchat/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openchat/node_modules/`.
...
1. Configuration globale (`~/.config/openchat/openchat.json`)
2. Configuration du projet (`openchat.json`)
3. Répertoire global des plugins (`~/.config/openchat/plugins/`)
4. Répertoire des plugins du projet (`.openchat/plugins/`)
...
- `client` : un client SDK openchat pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openchat.
...
Empêchez openchat de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openchat :
...
L'assistant `tool` crée un outil personnalisé que openchat peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openchat aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://openchat.ai/docs/sdk) pour plus de détails.
