...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\openchat\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `openchat --log-level DEBUG`.

---

## Stockage

openchat stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/openchat/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop exécute un serveur OpenCode local (le side-car `openchat-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/openchat/openchat.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Plugins mondiaux**
  ...
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\openchat\plugins`
...
- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/openchat`
- **Linux** : supprimez `~/.cache/openchat` (ou exécutez `rm -rf ~/.cache/openchat`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\openchat`
...
Si votre `openchat.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `OPENCHAT_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `OPENCHAT_PORT` (ou choisissez un port libre) et redémarrez.
...
- `openchat.settings.dat` (serveur de bureau par défaut URL)
- `openchat.global.dat` et `openchat.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
3. Assurez-vous d'avoir la dernière version avec `openchat upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `openchat models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\openchat`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. openchat installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\openchat`

2. Redémarrez openchat pour réinstaller les derniers packages du fournisseur

Cela forcera openchat à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
openchat détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
