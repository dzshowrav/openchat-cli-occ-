...
`~/.local/share/openchat/auth.json`
...
1. TUI에서 `/connect` 명령을 실행하고 `OpenCode Zen`을 선택한 뒤, [openchat.ai/auth](https://openchat.ai/zen)로 이동합니다.
...
1. TUI에서 `/connect` 명령을 실행하고 `OpenCode Go`를 선택한 뒤 [openchat.ai/auth](https://openchat.ai/zen)로 이동하십시오.
...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   프로젝트별 또는 영구 구성을 위해 `openchat.json`을 사용하십시오:
...
OpenAI 호환 API 서버 뒤에서 로컬 LLM을 실행하는 데스크톱 애플리케이션인 [Atomic Chat](https://atomic.chat)을 통해 로컬 모델을 사용하도록 openchat를 구성할 수 있습니다(기본 엔드포인트 `http://127.0.0.1:1337/v1`).
...
OpenCode를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `openchat.json` 파일에 다음을 추가하십시오.
...
Helicone의 [Sessions](https://docs.helicone.ai/features/sessions) 기능을 사용하면 관련 LLM 요청을 그룹화할 수 있습니다. [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) 플러그인을 사용하여 각 OpenCode 대화를 Helicone 세션으로 자동 기록하십시오.
...
:::tip
Ollama는 OpenCode에 대해 자동으로 구성될 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://docs.ollama.com/integrations/openchat)를 참조하십시오.
:::
...
4. 프로젝트 디렉토리에 `openchat.json` 파일을 만들거나 업데이트하십시오.
...
1. **인증 설정 확인**: `openchat auth list`를 실행하여 공급자의 자격 증명이 구성에 추가되었는지 확인하십시오.
