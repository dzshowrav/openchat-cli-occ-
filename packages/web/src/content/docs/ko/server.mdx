...
`openchat serve` 명령은 openchat 클라이언트가 사용할 수 있는 OpenAPI 엔드포인트를 노출하는 headless HTTP 서버를 실행합니다.
...
| `--mdns-domain` | mDNS 서비스의 사용자 지정 도메인 이름 | `openchat.local` |
...
`OPENCHAT_SERVER_PASSWORD`를 설정하여 서버를 HTTP Basic auth로 보호합니다. `openchat` 또는 `OPENCHAT_SERVER_USERNAME`를 오버라이드로 설정하는 사용자의 기본값. 이것은 `openchat serve`와 `openchat web` 둘 다에 적용합니다.
...
`openchat`를 실행하면 TUI와 서버를 시작합니다. TUI는 어디에 있습니까?
서버와 대화하는 클라이언트. 서버는 OpenAPI 3.1 spec을 노출
끝점. 이 엔드포인트는 [SDK](/docs/sdk)을 생성하는 데도 사용됩니다.

:::tip
openchat 서버를 사용하여 openchat programmatically와 상호 작용합니다.
:::

이 아키텍처는 openchat 지원 여러 클라이언트를 허용하고 openchat programmatically와 상호 작용 할 수 있습니다.

독립 서버를 시작하려면 `openchat serve`를 실행할 수 있습니다. 당신이 있는 경우에
openchat TUI 실행, `openchat serve` 새로운 서버를 시작합니다.
...
[`/tui`](#tui) 엔드포인트는 서버를 통해 TUI를 구동하는 데 사용될 수 있습니다. 예를 들어 미리 작성하거나 프롬프트를 실행할 수 있습니다. 이 설정은 openchat [IDE](/docs/ide) 플러그인에 의해 사용됩니다.
...
openchat 서버는 다음과 같은 API를 노출합니다.
