...
1. **Remote config**(`.well-known/openchat`) - 조직 기본값
2. **Global config**(`~/.config/openchat/openchat.json`) - 사용자 기본 설정
3. **Custom config**(`OPENCHAT_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `openchat.json`) - 프로젝트별 설정
5. **`.openchat` directories** - agents, commands, plugins
6. **Inline config**(`OPENCHAT_CONFIG_CONTENT` env var) - 런타임 override

즉, 프로젝트 config는 전역 기본값을 override할 수 있고, 전역 config는 조직의 Remote 기본값을 override할 수 있습니다.

:::note
`.openchat` 및 `~/.config/openchat` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
조직은 `.well-known/openchat` endpoint로 기본 config를 제공할 수 있습니다. 이를 지원하는 provider로 인증하면 자동으로 가져옵니다.
...
전역 OpenCode config는 `~/.config/openchat/openchat.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.

TUI 관련 설정은 `~/.config/openchat/tui.json`을 사용하세요.
...
프로젝트 루트에 `openchat.json`을 추가하세요. 프로젝트 config는 표준 config 파일 중 우선순위가 가장 높아 전역 및 Remote config를 모두 override합니다.
...
`OPENCHAT_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`OPENCHAT_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.openchat` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
custom 디렉토리는 전역 config와 `.openchat` 디렉토리 뒤에 로드되므로 해당 설정을 **override할 수 있습니다**.

---

## Schema

server/runtime config schema는 [**`openchat.ai/config.json`**](https://openchat.ai/config.json)에 정의되어 있습니다.

TUI config는 [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json)을 사용합니다.
...
`OPENCHAT_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.

`openchat.json`의 기존 `theme`, `keybinds`, `tui` 키는 더 이상 사용되지 않으며(deprecated) 가능한 경우 자동으로 마이그레이션됩니다.
...
`server` 옵션으로 `openchat serve`와 `openchat web` 명령의 server 설정을 구성할 수 있습니다.
...
- `mdnsDomain` - mDNS service의 custom 도메인 이름입니다. 기본값은 `openchat.local`입니다. 같은 네트워크에서 여러 인스턴스를 실행할 때 유용합니다.
...
`~/.config/openchat/agents/` 또는 `.openchat/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
이 설정은 TUI, CLI(`openchat run`), 데스크톱 앱, GitHub Action 등 모든 인터페이스에 적용됩니다.
...
`~/.config/openchat/commands/` 또는 `.openchat/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
plugin 파일은 `.openchat/plugins/` 또는 `~/.config/openchat/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
