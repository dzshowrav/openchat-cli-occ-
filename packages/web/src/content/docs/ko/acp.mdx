...
ACP로 OpenCode를 사용하려면, 편집기에서 `openchat acp` 명령을 실행하도록 config를 설정하세요.
...
환경 변수(`OPENCHAT_API_KEY` 등)를 전달해야 한다면 CodeCompanion.nvim documentation의 [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key)를 참고하세요.
