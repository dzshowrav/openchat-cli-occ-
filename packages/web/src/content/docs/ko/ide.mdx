...
OpenCode는 VS Code, Cursor, 또는 터미널을 지원하는 모든 IDE와 통합됩니다. 시작하려면 터미널에서 `openchat`를 실행하세요.
...
1. VS Code를 여세요.4
2. 통합 터미널을 여세요.
3. `openchat`를 실행하세요. 확장 프로그램이 자동으로 설치됩니다.
...
- 통합 터미널에서 `openchat`를 실행하고 있는지 확인하세요.
