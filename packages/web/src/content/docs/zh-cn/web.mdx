...
:::caution
如果未设置 `OPENCHAT_SERVER_PASSWORD`，服务器将没有安全保护。本地使用没有问题，但在网络访问时应当设置密码。
:::

:::tip[Windows 用户]
为获得最佳体验，建议从 [WSL](/docs/windows-wsl) 而非 PowerShell 运行 `openchat web`。这可以确保正确的文件系统访问和终端集成。
:::
...
这会自动将主机名设置为 `0.0.0.0`，并将服务器广播为 `openchat.local`。
...
要保护服务器访问，可以通过 `OPENCHAT_SERVER_PASSWORD` 环境变量设置密码：

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

用户名默认为 `openchat`，可以通过 `OPENCHAT_SERVER_USERNAME` 进行更改。
...
你也可以在 `openchat.json` 配置文件中设置服务器选项：
