...
1. **远程配置**（来自 `.well-known/openchat`）- 组织默认值
2. **全局配置**（`~/.config/openchat/openchat.json`）- 用户偏好
3. **自定义配置**（`OPENCHAT_CONFIG` 环境变量）- 自定义覆盖
4. **项目配置**（项目中的 `openchat.json`）- 项目特定设置
5. **`.openchat` 目录** - 代理、命令、插件
6. **内联配置**（`OPENCHAT_CONFIG_CONTENT` 环境变量）- 运行时覆盖

这意味着项目配置可以覆盖全局默认值，全局配置可以覆盖远程组织默认值。

:::note
`.openchat` 和 `~/.config/openchat` 目录的子目录使用**复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，也支持单数名称（例如 `agent/`）。
:::
...
组织可以通过 `.well-known/openchat` 端点提供默认配置。当您使用支持该功能的提供商进行身份验证时，会自动获取此配置。
...
将全局 OpenCode 配置放在 `~/.config/openchat/openchat.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
在项目根目录中添加 `openchat.json`。项目配置在标准配置文件中具有最高优先级——它会覆盖全局配置和远程配置。
...
使用 `OPENCHAT_CONFIG` 环境变量指定自定义配置文件路径。
...
使用 `OPENCHAT_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.openchat` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
...
自定义目录在全局配置和 `.openchat` 目录之后加载，因此**可以覆盖**它们的设置。

---

## Schema

配置文件具有在 [**`openchat.ai/config.json`**](https://openchat.ai/config.json) 中定义的 Schema。
...
您可以通过 `server` 选项为 `openchat serve` 和 `openchat web` 命令配置服务器设置。
...
- `mdnsDomain` - mDNS 服务的自定义域名。默认为 `openchat.local`。适用于在同一网络上运行多个实例的场景。
...
您还可以使用 `~/.config/openchat/agents/` 或 `.openchat/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
此设置适用于所有界面：TUI、CLI（`openchat run`）、桌面应用和 GitHub Action。
...
您还可以使用 `~/.config/openchat/commands/` 或 `.openchat/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
将插件文件放置在 `.openchat/plugins/` 或 `~/.config/openchat/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
