...
OpenCode 可以与你的 GitHub 工作流集成。在评论中提及 `/openchat` 或 `/oc`，OpenCode 就会在你的 GitHub Actions 运行器中执行任务。
...
1. **安装 GitHub App**
   ...
   前往 [**github.com/apps/openchat-agent**](https://github.com/apps/openchat-agent)，确保已在目标仓库中安装该应用。

2. **添加工作流**

   将以下工作流文件添加到仓库的 `.github/workflows/openchat.yml` 中。请确保在 `env` 中设置合适的 `model` 及所需的 API 密钥。
...
| 事件类型                      | 触发方式                     | 详情                                                                                      |
| ----------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------- |
| `issue_comment`               | 在 Issue 或 PR 上发表评论    | 在评论中提及 `/openchat` 或 `/oc`。OpenCode 会读取上下文，并可创建分支、提交 PR 或回复。  |
| `pull_request_review_comment` | 在 PR 中对特定代码行发表评论 | 在代码审查时提及 `/openchat` 或 `/oc`。OpenCode 会接收文件路径、行号和 diff 上下文。      |
