...
1. 在 TUI 中执行 `/connect` 命令，选择 openchat，然后前往 [openchat.ai/auth](https://openchat.ai/auth)。
...
2. 使用以下方法之一**配置身份验证**：
   ...
   ***
   ...
   如需项目级别或持久化的配置，请使用 `openchat.json`：
...
你可以通过 [Atomic Chat](https://atomic.chat) 配置 openchat 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `http://127.0.0.1:1337/v1`）。
...
:::note[合规说明]
OpenCode 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 OpenCode 仅使用你自己的 GitLab 托管实例，请在 `openchat.json` 文件中添加以下内容。同时建议禁用会话共享。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允许你将相关的 LLM 请求归为一组。使用 [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) 插件可以自动将每个 OpenCode 对话记录为 Helicone 中的一个会话。
...
:::tip
Ollama 可以自动为 OpenCode 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/openchat)。
:::
...
4. 在项目目录中创建或更新 `openchat.json` 文件：
...
1. **检查认证设置**：运行 `openchat auth list` 查看该提供商的凭据是否已添加到配置中。
