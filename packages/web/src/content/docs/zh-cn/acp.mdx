...
要通过 ACP 使用 OpenCode，请在编辑器中配置运行 `openchat acp` 命令。
...
如果需要传递环境变量（如 `OPENCHAT_API_KEY`），请参阅 CodeCompanion.nvim 文档中的[配置适配器：环境变量](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key)了解详细信息。
