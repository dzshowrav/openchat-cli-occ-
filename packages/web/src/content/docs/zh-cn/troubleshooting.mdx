...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.local\share\openchat\log`

日志文件以时间戳命名（例如 `2025-01-09T123456.log`），并保留最近的 10 个日志文件。

你可以通过 `--log-level` 命令行选项设置日志级别以获取更详细的调试信息。例如：`openchat --log-level DEBUG`。
...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop 会在后台运行一个本地 OpenCode 服务器（即 `openchat-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。
...
- **macOS/Linux**（旧版安装）: `~/.local/share/openchat/openchat.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **全局插件**
  ...
  - **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\openchat\plugins`
- **项目插件**（仅当你使用了项目级配置时）
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 粘贴 `~/.cache/openchat`
- **Linux**: 删除 `~/.cache/openchat`（或运行 `rm -rf ~/.cache/openchat`）
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.cache\openchat`
...
如果你的 `openchat.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。

#### 检查环境变量

如果你在环境中设置了 `OPENCHAT_PORT`，桌面应用将尝试使用该端口作为本地服务器端口。

- 取消设置 `OPENCHAT_PORT`（或选择一个空闲端口）并重新启动。
...
- `openchat.settings.dat`（桌面默认服务器 URL）
- `openchat.global.dat` 和 `openchat.workspace.*.dat`（UI 状态，如最近的服务器/项目）
...
3. 使用 `openchat upgrade` 确保你使用的是最新版本
...
要查看你有权访问哪些模型，请运行 `openchat models`
...
2. 如果问题仍然存在，请尝试清除已存储的配置：
   ...
   在 Windows 上，按 `WIN+R` 并删除：`%USERPROFILE%\.local\share\openchat`
...
1. 清除提供商包缓存：
   ...
   在 Windows 上，按 `WIN+R` 并删除：`%USERPROFILE%\.cache\openchat`
