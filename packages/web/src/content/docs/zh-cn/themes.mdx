...
默认情况下，OpenCode 使用我们自己的 `openchat` 主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/openchat/themes/*.json` 或 `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.openchat/themes/*.json`
4. **当前工作目录** — 定义在 `./.openchat/themes/*.json`
