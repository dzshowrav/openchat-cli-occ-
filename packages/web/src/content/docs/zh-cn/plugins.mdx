...
- `.openchat/plugins/` - 项目级插件
- `~/.config/openchat/plugins/` - 全局插件
...
**npm 插件**在启动时使用 Bun 自动安装。包及其依赖项会缓存在 `~/.cache/openchat/node_modules/` 中。
...
1. 全局配置 (`~/.config/openchat/openchat.json`)
2. 项目配置 (`openchat.json`)
3. 全局插件目录 (`~/.config/openchat/plugins/`)
4. 项目插件目录 (`.openchat/plugins/`)
...
日志级别：`debug`、`info`、`warn`、`error`。详情请参阅 [SDK 文档](https://openchat.ai/docs/sdk)。
