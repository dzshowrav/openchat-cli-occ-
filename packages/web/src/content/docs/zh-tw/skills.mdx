...
- 專案設定：`.openchat/skills/<name>/SKILL.md`
- 全域設定：`~/.config/openchat/skills/<name>/SKILL.md`
...
在此過程中，它會載入 `.openchat/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全域定義也會從 `~/.config/openchat/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中載入。
...
建立 `.openchat/skills/git-release/SKILL.md`，內容如下：
...
在 `openchat.json` 中使用基於模式的權限來控制代理可以存取哪些技能：
