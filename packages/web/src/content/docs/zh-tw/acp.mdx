...
要透過 ACP 使用 OpenCode，請在編輯器中設定執行 `openchat acp` 命令。
...
如果需要傳遞環境變數（如 `OPENCHAT_API_KEY`），請參閱 CodeCompanion.nvim 文件中的[設定適配器：環境變數](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key)了解詳細資訊。
