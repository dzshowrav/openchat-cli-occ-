...
預設情況下，OpenCode 使用我們自己的 `openchat` 主題。
...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/openchat/themes/*.json` 或 `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.openchat/themes/*.json`
4. **當前工作目錄** — 定義在 `./.openchat/themes/*.json`
