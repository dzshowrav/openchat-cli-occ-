...
OpenCode 可以與您的 GitHub 工作流程整合。在評論中提及 `/openchat` 或 `/oc`，OpenCode 就會在您的 GitHub Actions Runner 中執行任務。
...
1. **安裝 GitHub App**
   ...
   前往 [**github.com/apps/openchat-agent**](https://github.com/apps/openchat-agent)，確保已在目標儲存庫中安裝該應用程式。

2. **新增工作流程**

   將以下工作流程檔案新增到儲存庫的 `.github/workflows/openchat.yml` 中。請確保在 `env` 中設定合適的 `model` 及所需的 API 金鑰。
...
| 事件類型                      | 觸發方式                       | 詳情                                                                                      |
| ----------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------- |
| `issue_comment`               | 在 Issue 或 PR 上發表評論      | 在評論中提及 `/openchat` 或 `/oc`。OpenCode 會讀取上下文，並可建立分支、提交 PR 或回覆。  |
| `pull_request_review_comment` | 在 PR 中對特定程式碼行發表評論 | 在程式碼審查時提及 `/openchat` 或 `/oc`。OpenCode 會接收檔案路徑、行號和 diff 上下文。    |
