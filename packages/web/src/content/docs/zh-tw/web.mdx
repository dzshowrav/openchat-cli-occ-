...
:::caution
如果未設定 `OPENCHAT_SERVER_PASSWORD`，伺服器將沒有安全保護。本地使用沒有問題，但在網路存取時應當設定密碼。
:::

:::tip[Windows 使用者]
為獲得最佳體驗，建議從 [WSL](/docs/windows-wsl) 而非 PowerShell 執行 `openchat web`。這可以確保正確的檔案系統存取和終端機整合。
:::
...
這會自動將主機名稱設定為 `0.0.0.0`，並將伺服器廣播為 `openchat.local`。
...
要保護伺服器存取，可以透過 `OPENCHAT_SERVER_PASSWORD` 環境變數設定密碼：

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

使用者名稱預設為 `openchat`，可以透過 `OPENCHAT_SERVER_USERNAME` 進行更改。
...
你也可以在 `openchat.json` 設定檔中設定伺服器選項：
