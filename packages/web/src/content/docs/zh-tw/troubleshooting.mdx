...
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.local\share\openchat\log`

日誌檔案以時間戳記命名（例如 `2025-01-09T123456.log`），並保留最近的 10 個日誌檔案。

你可以透過 `--log-level` 命令列選項設定日誌等級以取得更詳細的偵錯資訊。例如：`openchat --log-level DEBUG`。
...
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop 會在背景執行一個本地 OpenCode 伺服器（即 `openchat-cli` 附屬程序）。大多數問題是由外掛異常、快取損壞或錯誤的伺服器設定引起的。
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc`（或 `~/.config/openchat/openchat.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/openchat/openchat.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **全域外掛**
  ...
  - **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\openchat\plugins`
- **專案外掛**（僅當你使用了專案級設定時）
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 貼上 `~/.cache/openchat`
- **Linux**: 刪除 `~/.cache/openchat`（或執行 `rm -rf ~/.cache/openchat`）
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.cache\openchat`
...
如果你的 `openchat.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。

#### 檢查環境變數

如果你在環境中設定了 `OPENCHAT_PORT`，桌面應用程式將嘗試使用該連接埠作為本地伺服器連接埠。

- 取消設定 `OPENCHAT_PORT`（或選擇一個空閒連接埠）並重新啟動。
...
- `openchat.settings.dat`（桌面預設伺服器 URL）
- `openchat.global.dat` 和 `openchat.workspace.*.dat`（UI 狀態，如最近的伺服器/專案）
...
3. 使用 `openchat upgrade` 確保你使用的是最新版本
...
要查看你有權存取哪些模型，請執行 `openchat models`
...
2. 如果問題仍然存在，請嘗試清除已儲存的設定：
   ...
   在 Windows 上，按 `WIN+R` 並刪除：`%USERPROFILE%\.local\share\openchat`
...
1. 清除供應商套件快取：
   ...
   在 Windows 上，按 `WIN+R` 並刪除：`%USERPROFILE%\.cache\openchat`
