...
- `.openchat/plugins/` - 專案級外掛
- `~/.config/openchat/plugins/` - 全域外掛
...
**npm 外掛**在啟動時使用 Bun 自動安裝。套件及其相依套件會快取在 `~/.cache/openchat/node_modules/` 中。
...
1. 全域設定 (`~/.config/openchat/openchat.json`)
2. 專案設定 (`openchat.json`)
3. 全域外掛目錄 (`~/.config/openchat/plugins/`)
4. 專案外掛目錄 (`.openchat/plugins/`)
...
日誌層級：`debug`、`info`、`warn`、`error`。詳情請參閱 [SDK 文件](https://openchat.ai/docs/sdk)。
