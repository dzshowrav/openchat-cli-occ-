...
`~/.local/share/openchat/auth.json` 中。
...
1. 在 TUI 中執行 `/connect` 指令，選擇 `OpenCode Zen`，然後前往 [openchat.ai/auth](https://openchat.ai/zen)。
...
1. 在 TUI 中執行 `/connect` 指令，選擇 `OpenCode Go`，然後前往 [openchat.ai/auth](https://openchat.ai/zen)。
...
2. 使用以下方法之一**設定身分驗證**：
   ...
   如需專案級別或持久化的設定，請使用 `openchat.json`：
...
你可以透過 [Atomic Chat](https://atomic.chat) 設定 openchat 以使用本地模型。Atomic Chat 是一款桌面應用程式，它在 OpenAI 相容的 API 伺服器後方執行本地 LLM（預設端點 `http://127.0.0.1:1337/v1`）。
...
:::note[合規說明]
OpenCode 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 OpenCode 僅使用您自己的 GitLab 託管實例，請在 `openchat.json` 檔案中新增以下內容。同時建議停用工作階段分享。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允許您將相關的 LLM 請求歸為一組。使用 [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) 外掛可以自動將每個 OpenCode 對話記錄為 Helicone 中的一個工作階段。
...
:::tip
Ollama 可以自動為 OpenCode 進行設定。詳見 [Ollama 整合文件](https://docs.ollama.com/integrations/openchat)。
:::
...
4. 在專案目錄中建立或更新 `openchat.json` 檔案：
...
1. **檢查認證設定**：執行 `openchat auth list` 查看該提供商的憑證是否已新增到設定中。
