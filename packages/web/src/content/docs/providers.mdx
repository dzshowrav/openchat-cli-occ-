...
in `~/.local/share/openchat/auth.json`.
...
1. Run the `/connect` command in the TUI, select `OpenCode Zen`, and head to [openchat.ai/auth](https://openchat.ai/zen).
...
1. Run the `/connect` command in the TUI, select `OpenCode Go`, and head to [openchat.ai/auth](https://openchat.ai/zen).
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running openchat:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `openchat.json`:
...
You can configure openchat to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for openchat to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your openchat config.
...
`openchat.json` file. It is also recommended to disable session sharing.
...
To use your GitHub Copilot subscription with openchat:
...
2. Set the required environment variables:
   ...
   Set them while running openchat.
...
In the event you see a feature or model from Helicone that isn't configured automatically through openchat, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) plugin to automatically log each OpenCode conversation as a session in Helicone.
...
You can configure openchat to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure openchat to use local models through LM Studio.
...
You can configure openchat to use local models through Ollama.

:::tip
Ollama can automatically configure itself for OpenCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/openchat) for details.
:::
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your openchat config.
...
5. You can also customize them through your openchat config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Openchat:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=openchat&utm_medium=docs&utm_campaign=cortex-provider).

openchat's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
For CI or headless environments, set a PAT or JWT before starting openchat:
...
The model catalog is provided automatically. A minimal `openchat.json` is all that's needed:
...
You can also customize models through your openchat config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with openchat. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `openchat.json` file in your project directory:
...
1. **Check the auth setup**: Run `openchat auth list` to see if the credentials
...
2. For custom providers, check the openchat config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your openchat config.
