...
С помощью openchat вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию openchat использует нашу собственную тему `openchat`.
...
openchat поставляется с несколькими встроенными темами.
...
- Хотите, чтобы openchat соответствовал внешнему виду их терминала
...
openchat поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/openchat/themes/*.json` или `$XDG_CONFIG_HOME/openchat/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.openchat/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.openchat/themes/*.json`.
...
```bash no-frame
mkdir -p ~/.config/openchat/themes
vim ~/.config/openchat/themes/my-theme.json
```

Для тем проекта:

```bash no-frame
mkdir -p .openchat/themes
vim .openchat/themes/my-theme.json
```
...
{
  "$schema": "https://openchat.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
