...
Создайте `.openchat/commands/test.md`:

```md title=".openchat/commands/test.md"
---
...
Вы можете добавлять собственные команды через конфигурацию openchat или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openchat [config](/docs/config):

```json title="openchat.jsonc" {4-12}
{
...
- Глобальный: `~/.config/openchat/commands/`
- Для каждого проекта: `.openchat/commands/`

```markdown title="~/.config/openchat/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openchat/commands/component.md"
---
...
```md title=".openchat/commands/create-file.md"
---
...
```md title=".openchat/commands/analyze-coverage.md"
---
...
```md title=".openchat/commands/review-changes.md"
---
...
```md title=".openchat/commands/review-component.md"
---
...
openchat включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
