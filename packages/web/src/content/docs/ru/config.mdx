---
title: Конфигурация
description: Использование конфигурации openchat JSON.
---

Вы можете настроить openchat, используя файл конфигурации JSON.

---

## Формат

openchat поддерживает форматы **JSON** и **JSONC** (JSON с комментариями).

```jsonc title="openchat.jsonc"
{
...
Например, если ваша глобальная конфигурация устанавливает `theme: "openchat"` и `autoupdate: true`, а конфигурация вашего проекта устанавливает `model: "anthropic/claude-sonnet-4-5"`, окончательная конфигурация будет включать все три параметра.
...
1. **Удаленная конфигурация** (от `.well-known/openchat`) – организационные настройки по умолчанию.
2. **Глобальная конфигурация** (`~/.config/openchat/openchat.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`OPENCHAT_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`openchat.json` в проекте) — настройки, специфичные для проекта.
5. **Каталоги `.openchat`** — агенты, команды, плагины
6. **Встроенная конфигурация** (`OPENCHAT_CONFIG_CONTENT` env var) – переопределяет время выполнения
...
:::note
В каталогах `.openchat` и `~/.config/openchat` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Организации могут предоставить конфигурацию по умолчанию через конечную точку `.well-known/openchat`. Он извлекается автоматически при аутентификации у провайдера, который его поддерживает.
...
```json title="Remote config from .well-known/openchat"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Поместите глобальную конфигурацию openchat в `~/.config/openchat/openchat.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.

Для настроек, специфичных для TUI, используйте `~/.config/openchat/tui.json`.
...
Добавьте `openchat.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Когда openchat запускается, он ищет файл конфигурации в текущем каталоге или переходит к ближайшему каталогу Git.
...
Укажите собственный путь к файлу конфигурации, используя переменную среды `OPENCHAT_CONFIG`.

```bash
export OPENCHAT_CONFIG=/path/to/my/custom-config.json
openchat run "Hello world"
```
...
Укажите пользовательский каталог конфигурации, используя переменную среды `OPENCHAT_CONFIG_DIR`. В этом каталоге будет выполняться поиск агентов, команд, режимов и плагинов (аналогично стандартному каталогу `.openchat`), и он должен иметь ту же структуру.

```bash
export OPENCHAT_CONFIG_DIR=/path/to/my/config-directory
openchat run "Hello world"
```

Пользовательский каталог загружается после каталогов global config и `.openchat`, поэтому он **может переопределить** их настройки.

---

## Схема

Файл конфигурации имеет схему, определенную в [**`openchat.ai/config.json`**](https://openchat.ai/config.json).

Конфигурация TUI использует [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json).
...
Используйте `OPENCHAT_TUI_CONFIG`, чтобы указать на пользовательский файл конфигурации TUI.

Устаревшие ключи `theme`, `keybinds` и `tui` в `openchat.json` устарели и автоматически переносятся, когда это возможно.
...
Вы можете настроить параметры сервера для команд `openchat serve` и `openchat web` с помощью опции `server`.
...
- `mdns` — включить обнаружение службы mDNS. Это позволит другим устройствам в сети обнаружить ваш сервер openchat.
- `mdnsDomain` — собственное доменное имя для службы mDNS. По умолчанию `openchat.local`. Полезно для запуска нескольких экземпляров в одной сети.
...
Вы можете настроить поставщиков и модели, которые хотите использовать в своей конфигурации openchat, с помощью параметров `provider`, `model` и `small_model`.
...
Опция `small_model` настраивает отдельную модель для облегченных задач, таких как создание заголовков. По умолчанию openchat пытается использовать более дешевую модель, если она доступна у вашего провайдера, в противном случае он возвращается к вашей основной модели.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/openchat/agents/` или `.openchat/agents/`. [Подробнее здесь](/docs/agents).
...
Агент по умолчанию должен быть основным агентом (а не субагентом). Это может быть встроенный агент, например `"build"` или `"plan"`, или [пользовательский агент](/docs/agents), который вы определили. Если указанный агент не существует или является субагентом, openchat вернется к `"build"` с предупреждением.

Этот параметр применяется ко всем интерфейсам: TUI, CLI (`openchat run`), настольному приложению и действию GitHub.
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/openchat/commands/` или `.openchat/commands/`. [Подробнее здесь](/docs/commands).
...
openchat автоматически загрузит все новые обновления при запуске. Вы можете отключить это с помощью опции `autoupdate`.
...
По умолчанию openchat **разрешает все операции**, не требуя явного разрешения. Вы можете изменить это, используя опцию `permission`.
...
[Плагины](/docs/plugins) расширяют openchat с помощью пользовательских инструментов, перехватчиков и интеграций.

Поместите файлы плагина в `.openchat/plugins/` или `~/.config/openchat/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.

```json title="openchat.json"
{
  "$schema": "https://openchat.ai/config.json",
  "plugin": ["openchat-helicone-session", "@my-org/custom-plugin"]
}
```
...
Это полезно, если вы хотите ограничить openchat использованием только определенных поставщиков, а не отключать их по одному.
