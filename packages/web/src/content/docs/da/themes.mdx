...
Som standard bruger OpenCode vårt eget `openchat`-tema.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openchat/themes/*.json` eller `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openchat/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openchat/themes/*.json`
