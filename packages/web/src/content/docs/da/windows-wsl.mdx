...
:::caution
Når du bruger `--hostname 0.0.0.0`, skal du sætte `OPENCHAT_SERVER_PASSWORD` for at sikre serveren.
...
1. **Kør `openchat web` i WSL-terminalen** i stedet for PowerShell:
...
Når du kører `openchat web` fra WSL, får du korrekt filsystemadgang og terminalintegration, samtidig med at det stadig er tilgængeligt fra din Windows-browser.
...
- Din OpenCode-konfiguration og dine sessioner gemmes i WSL-miljøet i `~/.local/share/openchat/`
