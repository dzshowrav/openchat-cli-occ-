...
Dette er adskilt fra `openchat.json`, som konfigurerer server-/kørselstidsadfærd.
...
Brug `OPENCHAT_TUI_CONFIG` til at indlæse en brugerdefineret TUI-konfigurationssti.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `openchat.default`.
