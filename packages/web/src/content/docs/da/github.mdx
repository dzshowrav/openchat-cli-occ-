...
OpenCode integreres med din GitHub arbejdsgang. Nævn `/openchat` eller `/oc` i din kommentar, og OpenCode vil udføre opgaver i din GitHub Actions-løber.
...
1. **Installationsprogrammet GitHub-appen**
   ...
   Gå over til [**github.com/apps/openchat-agent**](https://github.com/apps/openchat-agent). Sørg for, at det er installeret på mållageret.

2. **Tilføj arbejdsgangen**

   Tilføj følgende workflow-fil til `.github/workflows/openchat.yml` i din repo. Sørg for at indstille de relevante `model` og nødvendige API-nøgler i `env`.
...
| ----------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentarer og problemer eller PR       | Nævn `/openchat` eller `/oc` i din kommentar. OpenCode læser kontekst og kan oprette brancher, åbne PR'er eller svare.  |
| `pull_request_review_comment` | Kommenter specifikke kodelinjer i en PR | Nævn `/openchat` eller `/oc`, mens du gennemgår koden. OpenCode modtager filsti, linjenumre og diff-kontekst.           |
