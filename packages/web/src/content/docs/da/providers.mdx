...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openchat/auth.json`.
...
1. Kør kommandoen `/connect` i TUI, vælg openchat og gå til [openchat.ai/auth](https://openchat.ai/auth).
...
1. Kør kommandoen `/connect` i TUI, vælg `OpenCode Go`, og gå til [openchat.ai/auth](https://openchat.ai/zen).
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openchat:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `openchat.json`:
...
Du kan konfigurere openchat til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openchat skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem openchat-konfigurationen.
...
gemme tokenet i openchat auth-lagring.
:::
...
`openchat.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører openchat.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openchat, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) plugin for automatisk at logge hver OpenCode-samtale som en session i Helicone.
...
Du kan konfigurere openchat til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openchat til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openchat til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openchat) for detaljer.
:::
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem openchat-konfigurationen.
...
5. Du kan også tilpasse dem gennem openchat-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Du kan også tilpasse modeller gennem openchat-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openchat. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `openchat.json`-filen i projektkataloget:
...
1. **Tjek godkendelsesopsætningen**: Kør `openchat auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek openchat-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i openchat-konfigurationen.
