...
:::note
Dette verktøyet er bare tilgængelig når `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `OPENCHAT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
