...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openchat\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `openchat --log-level DEBUG`.

---

## Lagring

openchat gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop kører en lokal OpenCode-server (`openchat-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (eller `~/.config/openchat/openchat.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openchat\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/openchat`
- **Linux**: slet `~/.cache/openchat` (eller kør `rm -rf ~/.cache/openchat`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\openchat`
...
Hvis `openchat.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OPENCHAT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENCHAT_PORT` (eller vælg en ledig port) og start på ny.
...
- `openchat.settings.dat` (skrivebordsstandardserver URL)
- `openchat.global.dat` og `openchat.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
3. Sørg for at du har den nyeste version med `openchat upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `openchat models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\openchat`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. openchat installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\openchat`

2. Start openchat på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge openchat til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
openchat vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
