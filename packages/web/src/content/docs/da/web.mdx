...
:::caution
Hvis `OPENCHAT_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::

:::tip[Windows-brugere]
For den bedste oplevelse skal du køre `openchat web` fra [WSL](/docs/windows-wsl) i stedet for PowerShell. Dette sikrer korrekt filsystemadgang og terminalintegration.
:::
...
Dette indstiller automatisk værtsnavnet til `0.0.0.0` og annoncerer serveren som `openchat.local`.
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `OPENCHAT_SERVER_PASSWORD` miljøvariablen:

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

Brugernavnet er som standard `openchat`, men kan ændres med `OPENCHAT_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `openchat.json`-konfigurationsfil:
