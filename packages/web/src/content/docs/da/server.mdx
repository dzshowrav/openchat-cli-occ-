---
title: Server
description: Interager med openchat-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openchat serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openchat-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openchat.local` |
...
Indstil `OPENCHAT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openchat`, eller indstil `OPENCHAT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openchat serve` og `openchat web`.
...
Når du kører `openchat` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openchat-serveren til at interagere med openchat programmatisk.
:::

Denne arkitektur lader openchat understøtte flere klienter og lader dig interagere med openchat programmatisk.

Du kan køre `openchat serve` for at starte en selvstændig server. Hvis du har
openchat TUI kørende, vil `openchat serve` starte en ny server.
