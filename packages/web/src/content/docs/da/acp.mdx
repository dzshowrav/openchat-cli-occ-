...
For at bruge OpenCode via ACP, konfigurer din editor til at køre kommandoen `openchat acp`.
...
Hvis du har brug for at sende miljøvariabler (som `OPENCHAT_API_KEY`), henvises til [Konfiguration af adaptere: Miljøvariabler](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) i CodeCompanion.nvim-dokumentationen for alle detaljer.
