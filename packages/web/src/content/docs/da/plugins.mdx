...
- `.openchat/plugins/` - Plugins på projektniveau
- `~/.config/openchat/plugins/` - Globale plugins
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openchat/node_modules/`.
...
1. Global konfiguration (`~/.config/openchat/openchat.json`)
2. Projektkonfiguration (`openchat.json`)
3. Global plugin-mappe (`~/.config/openchat/plugins/`)
4. Projekt plugin bibliotek (`.openchat/plugins/`)
...
- `client`: En openchat SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openchat.
...
Undgå openchat i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openchat:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openchat kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openchat sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://openchat.ai/docs/sdk) for detaljer.
