...
1. **Fjernkonfiguration** (fra `.well-known/openchat`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/openchat/openchat.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCHAT_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`openchat.json` i projekt) - projektspecifikke indstillinger
5. **`.openchat` mapper** - agent, kommandoer, plugins
6. **Inline config** (`OPENCHAT_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.openchat` og `~/.config/openchat` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/openchat`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenCode-konfiguration i `~/.config/openchat/openchat.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `openchat.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `OPENCHAT_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `OPENCHAT_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.openchat` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.openchat` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`openchat.ai/config.json`**](https://openchat.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `openchat serve` og `openchat web` gennem indstillingen `server`.
...
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `openchat.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/openchat/agents/` eller `.openchat/agents/`. [Learn more here](/docs/agents).
...
Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`openchat run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/openchat/commands/` eller `.openchat/commands/`. [Learn more here](/docs/commands).
...
Som standard openchat **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
Placer plugin-filer i `.openchat/plugins/` eller `~/.config/openchat/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
