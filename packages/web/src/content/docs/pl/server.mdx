---
title: Serwer
description: Interakcja z serwerem openchat za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openchat serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openchat.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openchat.local` |
...
Ustaw `OPENCHAT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openchat` lub ustaw `OPENCHAT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openchat serve`, jak i `openchat web`.
...
Kiedy uruchomisz `openchat`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openchat do programowej interakcji z openchat.
:::

Ta architektura umożliwia obsługę wielu klientów przez openchat i programową interakcję z kodem otwartym.

Możesz uruchomić `openchat serve`, aby uruchomić samodzielny serwer. Jeśli masz
openchat TUI działa, `openchat serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openchat [IDE](/docs/ide).
...
Serwer openchat udostępnia następujące interfejsy API.
