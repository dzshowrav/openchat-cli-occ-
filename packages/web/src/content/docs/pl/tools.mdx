...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCode lub gdy zmienna środowiskowa `OPENCHAT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).
