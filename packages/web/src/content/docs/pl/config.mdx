...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "openchat"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/openchat`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/openchat/openchat.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCHAT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`openchat.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.openchat`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OPENCHAT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.openchat` i `~/.config/openchat` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/openchat`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/openchat/openchat.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/openchat/tui.json`.
...
Dodaj `openchat.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OPENCHAT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCHAT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.openchat` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.openchat`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`openchat.ai/config.json`**](https://openchat.ai/config.json).
...
Użyj `OPENCHAT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `openchat.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `openchat serve` i `openchat web` za pomocą opcji `server`.
...
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `openchat.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/openchat/agents/` lub `.openchat/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Dotyczy to wszystkich interfejsów: TUI, CLI (`openchat run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/openchat/commands/` lub `.openchat/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.openchat/plugins/` lub `~/.config/openchat/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
