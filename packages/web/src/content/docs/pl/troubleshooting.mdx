...
Aby debugować problemy z openchat, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openchat\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `openchat --log-level DEBUG`.

---

## Przechowywanie danych

openchat przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openchat`
...
openchat Desktop uruchamia w tle lokalny serwer openchat (sidecar `openchat-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (lub `~/.config/openchat/openchat.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
openchat może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/openchat/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openchat\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby openchat mógł ją odbudować.

1. Całkowicie zamknij openchat Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/openchat`
- **Linux**: usuń `~/.cache/openchat` (lub uruchom `rm -rf ~/.cache/openchat`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\openchat`

3. Uruchom ponownie openchat Desktop.

---

### Napraw problemy z połączeniem z serwerem

openchat Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `openchat.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OPENCHAT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OPENCHAT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows openchat Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami openchat.

---

### Brak powiadomień

openchat Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla openchat w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie openchat Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji openchat Desktop):

- `openchat.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `openchat.global.dat` i `openchat.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z openchat:
...
3. Upewnij się, że masz najnowszą wersję z `openchat upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `openchat models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\openchat`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. openchat dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\openchat`

2. Uruchom ponownie kod openchat, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to openchat do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
openchat wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
