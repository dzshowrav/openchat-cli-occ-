...
Dzięki openchat możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openchat używa naszego własnego motywu `openchat`.
...
openchat ma kilka wbudowanych motywów.
...
openchat obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openchat/themes/*.json` lub `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openchat/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openchat/themes/*.json`
