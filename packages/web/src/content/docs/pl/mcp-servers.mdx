...
Możesz dodać dodatkowe informacje do openchat za pomocą _Model Context Protocol_, w skrócie MCP. openchat obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openchat Config](https://openchat.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openchat`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openchat automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openchat:
...
Jeśli serwer wymaga uwierzytelnienia, openchat poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openchat mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openchat bezpieczne przechowa tokeny w `~/.local/share/openchat/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openchat, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openchat, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openchat z kontem Sentry.
