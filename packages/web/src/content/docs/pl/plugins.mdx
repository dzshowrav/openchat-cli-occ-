---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openchat.
---

Wtyczki umożliwiające wyłączenie openchat poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openchat.
...
- `.openchat/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openchat/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openchat/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openchat/openchat.json`)
2. Konfiguracja projektu (`openchat.json`)
3. Globalny katalog wtyczek (`~/.config/openchat/plugins/`)
4. Katalog wtyczek projektu (`.openchat/plugins/`)
...
openchat uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openchat do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openchat.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openchat, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openchat czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openchat:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openchat. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://openchat.ai/docs/sdk).
