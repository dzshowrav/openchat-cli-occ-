---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openchat.
---

import config from "../../../../config.mjs"
export const console = config.console

openchat używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openchat.
...
w `~/.local/share/openchat/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openchat
config.
...
OpenCode Zen to lista modeli dostarczonych przez zespół openchat, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openchat. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Zen` i przejdź do [openchat.ai/auth](https://openchat.ai/zen).
...
Działa jak każdy inny dostawca w openchat i jest całkowicie opcjonalny w użyciu.

---

## OpenCode Go

OpenCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół openchat, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z openchat.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Go` i przejdź do [openchat.ai/auth](https://openchat.ai/zen).
...
Aby używać Amazon Bedrock z openchat:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openchat:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openchat.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openchat nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować openchat do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openchat działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openchat.
...
openchat używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openchat
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openchat.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `openchat.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openchat:
...
Aby używać Google Vertex AI z openchat:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania openchat.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openchat, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session), aby automatycznie rejestrować każdą konwersację openchat jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openchat będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openchat tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openchat tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openchat tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openchat. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openchat).
:::
...
Aby korzystać z Ollama Cloud z openchat:
...
3. Skopiuj klucz API do użycia w openchat.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openchat musisz pobrać informacje o modelu lokalnie:
...
OpenCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openchat. [Dowiedz się więcej](/docs/zen).
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openchat.
...
5. Możesz je także dostosować za pomocą konfiguracji openchat. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openchat:
...
Możesz także dostosować modele za pomocą konfiguracji openchat. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openchat. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openchat.json` w katalogu projektu:
...
Pola `limit` pozwalają openchat zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openchat auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openchat i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openchat.
