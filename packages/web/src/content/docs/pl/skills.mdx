...
Umiejętności agenta pozwalają openchat odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openchat przeszukuje te lokalizacje:

- Project config: `.openchat/skills/<name>/SKILL.md`
- Global config: `~/.config/openchat/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openchat przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openchat/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openchat/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openchat/skills/git-release/SKILL.md` w ten sposób:
...
openchat wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openchat.json`:
...
**Dla agentów wbudowanych** (w `openchat.json`):
