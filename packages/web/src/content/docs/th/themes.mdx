...
ตามค่าเริ่มต้น OpenCode จะใช้ธีม `openchat` ของเราเอง
...
2. **ไดเรกทอรีกำหนดค่าผู้ใช้** - กำหนดใน `~/.config/openchat/themes/*.json` หรือ `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **ไดเรกทอรีรากของโครงการ** - กำหนดไว้ใน `<project-root>/.openchat/themes/*.json`
4. **ไดเร็กทอรีการทำงานปัจจุบัน** - กำหนดใน `./.openchat/themes/*.json`
