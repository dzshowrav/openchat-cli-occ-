...
1. **การกำหนดค่าระยะไกล** (จาก `.well-known/openchat`) - ค่าเริ่มต้นขององค์กร
2. **การกำหนดค่าสากล** (`~/.config/openchat/openchat.json`) - การตั้งค่าของผู้ใช้
3. **การกำหนดค่าแบบกำหนดเอง** (`OPENCHAT_CONFIG` env var) - การแทนที่แบบกำหนดเอง
4. **การกำหนดค่าโครงการ** (`openchat.json` ในโครงการ) - การตั้งค่าเฉพาะโครงการ
5. **`.openchat` ไดเรกทอรี** - ตัวแทน คำสั่ง ปลั๊กอิน
6. **การกำหนดค่าแบบอินไลน์** (`OPENCHAT_CONFIG_CONTENT` env var) - การแทนที่รันไทม์
...
:::note
ไดเรกทอรี `.openchat` และ `~/.config/openchat` ใช้ **ชื่อพหูพจน์** สำหรับไดเรกทอรีย่อย: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` และ `themes/` ชื่อเอกพจน์ (เช่น `agent/`) ยังรองรับความเข้ากันได้แบบย้อนหลังด้วย
:::
...
องค์กรสามารถจัดเตรียมการกำหนดค่าเริ่มต้นผ่านทางจุดสิ้นสุด `.well-known/openchat` สิ่งนี้จะถูกดึงออกมาโดยอัตโนมัติเมื่อคุณตรวจสอบสิทธิ์กับผู้ให้บริการที่รองรับ
...
วางการกำหนดค่า OpenCode ส่วนกลางของคุณใน `~/.config/openchat/openchat.json` ใช้การกำหนดค่าส่วนกลางสำหรับการตั้งค่าทั้งผู้ใช้ เช่น ผู้ให้บริการ รุ่น และสิทธิ์

สำหรับการตั้งค่าเฉพาะ TUI ให้ใช้ `~/.config/openchat/tui.json`
...
เพิ่ม `openchat.json` ในรูทโปรเจ็กต์ของคุณ การกำหนดค่าโปรเจ็กต์มีความสำคัญสูงสุดในบรรดาไฟล์กำหนดค่ามาตรฐาน โดยจะแทนที่การกำหนดค่าทั้งส่วนกลางและระยะไกล
...
ระบุเส้นทางไฟล์กำหนดค่าที่กำหนดเองโดยใช้ตัวแปรสภาพแวดล้อม `OPENCHAT_CONFIG`
...
ระบุไดเร็กทอรีการกำหนดค่าที่กำหนดเองโดยใช้ `OPENCHAT_CONFIG_DIR`
ตัวแปรสภาพแวดล้อม ไดเร็กทอรีนี้จะถูกค้นหาตัวแทน, คำสั่ง,
โหมดและปลั๊กอินเหมือนกับไดเร็กทอรี `.openchat` มาตรฐานและควร
...
ไดเร็กทอรีที่กำหนดเองถูกโหลดหลังจากไดเร็กทอรี config ส่วนกลางและ `.openchat` ดังนั้นจึง **สามารถแทนที่** การตั้งค่าได้

---

## สคีมา

ไฟล์กำหนดค่ามีสคีมาที่กำหนดไว้ใน [**`openchat.ai/config.json`**](https://openchat.ai/config.json)

การกำหนดค่า TUI ใช้ [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json)
...
ใช้ `OPENCHAT_TUI_CONFIG` เพื่อชี้ไปยังไฟล์กำหนดค่า TUI ที่กำหนดเอง

คีย์ `theme`, `keybinds` และ `tui` แบบเดิมใน `openchat.json` เลิกใช้แล้วและจะถูกย้ายโดยอัตโนมัติเมื่อเป็นไปได้
...
คุณสามารถกำหนดการตั้งค่าเซิร์ฟเวอร์สำหรับคำสั่ง `openchat serve` และ `openchat web` ผ่านตัวเลือก `server`
...
- `mdnsDomain` - ​​ชื่อโดเมนที่กำหนดเองสำหรับบริการ mDNS ค่าเริ่มต้นเป็น `openchat.local` มีประโยชน์สำหรับการเรียกใช้หลายอินสแตนซ์บนเครือข่ายเดียวกัน
...
คุณยังสามารถกำหนดตัวแทนโดยใช้ไฟล์ Markdown ใน `~/.config/openchat/agents/` หรือ `.openchat/agents/` [เรียนรู้เพิ่มเติมที่นี่](/docs/agents)
...
การตั้งค่านี้ใช้กับอินเทอร์เฟซทั้งหมด: TUI, CLI (`openchat run`) แอปเดสก์ท็อป และ GitHub Action
...
คุณยังสามารถกำหนดคำสั่งโดยใช้ไฟล์ Markdown ใน `~/.config/openchat/commands/` หรือ `.openchat/commands/` [เรียนรู้เพิ่มเติมที่นี่](/docs/commands)
...
ตามค่าเริ่มต้น openchat **อนุญาตการดำเนินการทั้งหมด** โดยไม่ต้องมีการอนุมัติอย่างชัดเจน คุณสามารถเปลี่ยนสิ่งนี้ได้โดยใช้ตัวเลือก `permission`
...
วางไฟล์ปลั๊กอินใน `.openchat/plugins/` หรือ `~/.config/openchat/plugins/` คุณยังสามารถโหลดปลั๊กอินจาก npm ผ่านตัวเลือก `plugin`
