...
ใน `~/.local/share/openchat/auth.json`.
...
1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก openchat และไปที่ [openchat.ai/auth](https://openchat.ai/auth)
...
1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก `OpenCode Go` และไปที่ [openchat.ai/auth](https://openchat.ai/zen)
...
2. **กำหนดค่าการตรวจสอบสิทธิ์** โดยใช้วิธีใดวิธีหนึ่งต่อไปนี้:
   ...
   ตั้งค่าหนึ่งในตัวแปรสภาพแวดล้อมเหล่านี้ขณะเรียกใช้ openchat:
   ...
   สำหรับการกำหนดค่าเฉพาะโครงการหรือแบบถาวร ให้ใช้ `openchat.json`:
...
คุณสามารถกำหนดค่า openchat ให้ใช้โมเดลท้องถิ่นผ่าน [Atomic Chat](https://atomic.chat) ซึ่งเป็นแอปพลิเคชันเดสก์ท็อปที่เรียกใช้ LLM ในเครื่องภายใต้เซิร์ฟเวอร์ API ที่เข้ากันได้กับ OpenAI (ปลายทางเริ่มต้น `http://127.0.0.1:1337/v1`)
...
2. ไปที่ [Azure AI Foundry](https://ai.azure.com/) และปรับใช้โมเดล
   ...
   :::note
   ชื่อการปรับใช้จะต้องตรงกับชื่อรุ่นเพื่อให้ openchat ทำงานได้อย่างถูกต้อง
   :::
...
5. รันคำสั่ง `/models` เพื่อเลือกรุ่น
   ...
   คุณยังสามารถเพิ่มโมเดลผ่านการกำหนดค่า openchat ของคุณได้
...
เพื่อจัดเก็บโทเค็นในที่เก็บข้อมูลการรับรองความถูกต้องของ openchat
:::
...
`openchat.json` ไฟล์. ขอแนะนำให้ปิดใช้งานการแชร์เซสชันด้วย
...
วิธีใช้การสมัครสมาชิก GitHub Copilot ด้วย openchat:
...
2. ตั้งค่าตัวแปรสภาพแวดล้อมที่ต้องการ:
   ...
   ตั้งค่าในขณะที่เรียกใช้ openchat
...
ในกรณีที่คุณเห็นคุณสมบัติหรือรุ่นจาก Helicone ที่ไม่ได้กำหนดค่าโดยอัตโนมัติผ่าน openchat คุณสามารถกำหนดค่าด้วยตนเองได้ตลอดเวลา
...
คุณลักษณะ [เซสชัน](https://docs.helicone.ai/features/sessions) ของ Helicone ช่วยให้คุณสามารถจัดกลุ่มคำขอ LLM ที่เกี่ยวข้องเข้าด้วยกันได้ ใช้ปลั๊กอิน [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) เพื่อบันทึกการสนทนา OpenCode แต่ละรายการเป็นเซสชันใน Helicone โดยอัตโนมัติ
...
คุณสามารถกำหนดค่า openchat เพื่อใช้โมเดลท้องถิ่นผ่านยูทิลิตี้ [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server
...
คุณสามารถกำหนดค่า openchat เพื่อใช้โมเดลท้องถิ่นผ่าน LM Studio
...
คุณสามารถกำหนดค่า openchat เพื่อใช้โมเดลท้องถิ่นผ่าน Ollama

:::tip
Ollama สามารถกำหนดค่าตัวเองสำหรับ OpenCode ได้โดยอัตโนมัติ ดู [เอกสารการบูรณาการ Ollama](https://docs.ollama.com/integrations/openchat) เพื่อดูรายละเอียด
:::
...
4. โมเดล OpenRouter จำนวนมากถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น รันคำสั่ง `/models` เพื่อเลือกโมเดลที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า openchat ของคุณได้
...
5. คุณยังสามารถปรับแต่งได้ผ่านการกำหนดค่า openchat ของคุณ นี่คือตัวอย่างการระบุผู้ให้บริการ
...
วิธีใช้ [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) กับ Openchat:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า openchat ของคุณได้ นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI กับ openchat ได้ ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `openchat.json` ของคุณในไดเร็กทอรีโครงการของคุณ:
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: รัน `openchat auth list` เพื่อดูว่ามีการเพิ่มข้อมูลรับรอง
...
2. สำหรับผู้ให้บริการที่กำหนดเอง ให้ตรวจสอบการกำหนดค่า openchat และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า openchat ของคุณ
...
4. OpenRouter หลายรุ่นถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น ให้รันคำสั่ง `/models` เพื่อเลือกรุ่นที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า openchat ของคุณ
...
วิธีใช้ [Scaleway Generative API](https://www.scaleway.com/en/docs/generative-apis/) กับ Openchat:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า openchat ของคุณได้อีกด้วย นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางของผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI ด้วย openchat ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `openchat.json` ในไดเรกทอรีโครงการของคุณ:
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: เรียกใช้ `openchat auth list` เพื่อดูว่ามีข้อมูลรับรองหรือไม่
...
2. สำหรับผู้ให้บริการแบบกำหนดเอง ให้ตรวจสอบการกำหนดค่า openchat และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า openchat ของคุณ
