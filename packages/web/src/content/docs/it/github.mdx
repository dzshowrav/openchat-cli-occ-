...
OpenCode si integra nel tuo workflow GitHub. Menziona `/openchat` o `/oc` in un commento e OpenCode eseguira' i task dentro il runner di GitHub Actions.
...
1. **Installa la GitHub app**
   ...
   Vai su [**github.com/apps/openchat-agent**](https://github.com/apps/openchat-agent). Assicurati che sia installata sul repository di destinazione.

2. **Aggiungi il workflow**

   Aggiungi il seguente file workflow in `.github/workflows/openchat.yml` nel tuo repo. Assicurati di impostare il `model` appropriato e le API key richieste in `env`.
...
| ----------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Commento su una issue o PR             | Menziona `/openchat` o `/oc` nel commento. OpenCode legge il contesto e puo' creare branch, aprire PR o rispondere.       |
| `pull_request_review_comment` | Commento su specifiche righe in una PR | Menziona `/openchat` o `/oc` durante una review. OpenCode riceve path file, numeri di riga e contesto del diff.           |
