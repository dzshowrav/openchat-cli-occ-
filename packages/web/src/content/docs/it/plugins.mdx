...
- `.openchat/plugins/` - plugin a livello progetto
- `~/.config/openchat/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openchat/node_modules/`.
...
1. Config globale (`~/.config/openchat/openchat.json`)
2. Config di progetto (`openchat.json`)
3. Directory plugin globale (`~/.config/openchat/plugins/`)
4. Directory plugin di progetto (`.openchat/plugins/`)
...
- `client`: un client SDK di openchat per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openchat.
...
Impedisci a openchat di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openchat:
...
L'helper `tool` crea uno strumento personalizzato che openchat può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openchat insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://openchat.ai/docs/sdk) per i dettagli.
