...
Per impostazione predefinita, OpenCode usa il tema `openchat`.
...
2. **Directory di configurazione utente** - in `~/.config/openchat/themes/*.json` o `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.openchat/themes/*.json`
4. **Directory di lavoro corrente** - in `./.openchat/themes/*.json`
