---
title: Server
description: Interagisci con il server openchat via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openchat serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openchat.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openchat.local` |
...
Imposta `OPENCHAT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openchat`, oppure imposta `OPENCHAT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openchat serve` sia per `openchat web`.
...
Quando esegui `openchat` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openchat per interagire con openchat in modo programmatico.
:::

Questa architettura permette a openchat di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openchat serve` per avviare un server standalone. Se la TUI di openchat e gia in esecuzione, `openchat serve` avviera un nuovo server.
...
Il server openchat espone le seguenti API.
