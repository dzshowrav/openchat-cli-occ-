...
Configura gli agenti nel file `openchat.json`:
...
- Globale: `~/.config/openchat/agents/`
- Per progetto: `.openchat/agents/`
...
L'ID modello nella configurazione OpenCode usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](/docs/zen), useresti `openchat/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `openchat models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/openchat).
:::
