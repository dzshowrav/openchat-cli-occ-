...
:::caution
Se `OPENCHAT_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::

:::tip[Utenti Windows]
Per la migliore esperienza, esegui `openchat web` da [WSL](/docs/windows-wsl) invece che da PowerShell. Questo garantisce accesso corretto al file system e integrazione col terminale.
:::
...
Questo imposta automaticamente l'hostname a `0.0.0.0` e pubblicizza il server come `openchat.local`.
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `OPENCHAT_SERVER_PASSWORD`:

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

Lo username predefinito e `openchat`, ma puo essere cambiato con `OPENCHAT_SERVER_USERNAME`.
...
Puoi anche configurare le impostazioni del server nel file di config `openchat.json`:
