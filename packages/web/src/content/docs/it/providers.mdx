...
1. Esegui `/connect` nella TUI, seleziona openchat e vai su [openchat.ai/auth](https://openchat.ai/auth).
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui openchat:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `openchat.json`:
...
Puoi configurare openchat per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché openchat funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di openchat.
...
salvare il token nello storage di auth di openchat.
:::
...
file `openchat.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `openchat.json`:
...
Per usare il tuo abbonamento GitHub Copilot con openchat:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui openchat.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da openchat, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) per loggare automaticamente ogni conversazione di OpenCode come una sessione in Helicone.
...
Puoi configurare openchat per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare openchat per usare modelli locali tramite LM Studio.
...
Puoi configurare openchat per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/openchat) per dettagli.
:::
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di openchat.
...
5. Puoi anche personalizzarli tramite la configurazione di openchat. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Openchat:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di openchat. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con openchat. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `openchat.json` nella directory del progetto:
...
1. **Controlla il setup auth**: Esegui `openchat auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di openchat e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config openchat.
