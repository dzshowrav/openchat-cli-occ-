...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\openchat\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `openchat --log-level DEBUG`.

---

## Archiviazione

openchat salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop esegue in background un server locale di OpenCode (il sidecar `openchat-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\openchat\plugins`
...
- **Linux**: elimina `~/.cache/openchat` (oppure esegui `rm -rf ~/.cache/openchat`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\openchat`
...
Se il tuo `openchat.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OPENCHAT_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OPENCHAT_PORT` (o scegli una porta libera) e riavvia.
...
- `openchat.settings.dat` (desktop default server URL)
- `openchat.global.dat` e `openchat.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
3. Assicurati di avere l'ultima versione con `openchat upgrade`
...
Per capire a quali modelli hai accesso, esegui `openchat models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\openchat`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. openchat installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\openchat`

2. Riavvia openchat per reinstallare i pacchetti provider piu recenti

Questo forzera openchat a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
openchat rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
