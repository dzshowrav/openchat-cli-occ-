...
1. **Config remota** (da `.well-known/openchat`) - default dell'organizzazione
2. **Config globale** (`~/.config/openchat/openchat.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCHAT_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`openchat.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.openchat`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `OPENCHAT_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.openchat` e `~/.config/openchat` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/openchat`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di OpenCode in `~/.config/openchat/openchat.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/openchat/tui.json`.
...
Aggiungi `openchat.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `OPENCHAT_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCHAT_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.openchat` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.openchat`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`openchat.ai/config.json`**](https://openchat.ai/config.json).

La configurazione TUI usa [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json).
...
Usa `OPENCHAT_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `openchat.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `openchat serve` e `openchat web` tramite l'opzione `server`.
...
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `openchat.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi anche definire agenti usando file markdown in `~/.config/openchat/agents/` o `.openchat/agents/`. [Scopri di piu](/docs/agents).
...
Questa impostazione si applica a tutte le interfacce: TUI, CLI (`openchat run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/openchat/commands/` o `.openchat/commands/`. [Scopri di piu](/docs/commands).
...
Per impostazione predefinita, openchat **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
Metti i file dei plugin in `.openchat/plugins/` o `~/.config/openchat/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
