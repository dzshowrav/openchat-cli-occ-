...
Questo è separato da `openchat.json`, che configura il comportamento di server/runtime.
...
Usa `OPENCHAT_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `openchat.default`.
