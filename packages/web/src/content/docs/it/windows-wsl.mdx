...
:::caution
Quando usi `--hostname 0.0.0.0`, imposta `OPENCHAT_SERVER_PASSWORD` per proteggere il server.
...
1. **Esegui `openchat web` nel terminale WSL** invece che in PowerShell:
...
Eseguire `openchat web` da WSL garantisce accesso corretto al file system e integrazione del terminale, restando comunque accessibile dal browser Windows.
...
- Configurazione e sessioni di OpenCode sono salvate nell'ambiente WSL in `~/.local/share/openchat/`
