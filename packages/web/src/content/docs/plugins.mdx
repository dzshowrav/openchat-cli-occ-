...
- `.openchat/plugins/` - Project-level plugins
- `~/.config/openchat/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/openchat/node_modules/`.
...
1. Global config (`~/.config/openchat/openchat.json`)
2. Project config (`openchat.json`)
3. Global plugin directory (`~/.config/openchat/plugins/`)
4. Project plugin directory (`.openchat/plugins/`)
...
- `client`: An openchat SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend openchat.
...
Prevent openchat from reading `.env` files:
...
Plugins can also add custom tools to openchat:
...
The `tool` helper creates a custom tool that openchat can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to openchat alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://openchat.ai/docs/sdk) for details.
