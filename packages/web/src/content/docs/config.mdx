...
1. **Remote config** (from `.well-known/openchat`) - organizational defaults
2. **Global config** (`~/.config/openchat/openchat.json`) - user preferences
3. **Custom config** (`OPENCHAT_CONFIG` env var) - custom overrides
4. **Project config** (`openchat.json` in project) - project-specific settings
5. **`.openchat` directories** - agents, commands, plugins
6. **Inline config** (`OPENCHAT_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/openchat/` on macOS) - admin-controlled
...
:::note
The `.openchat` and `~/.config/openchat` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/openchat` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global OpenCode config in `~/.config/openchat/openchat.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/openchat/tui.json`.
...
Add `openchat.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `OPENCHAT_CONFIG` environment variable.
...
Specify a custom config directory using the `OPENCHAT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.openchat` directory, and should
...
The custom directory is loaded after the global config and `.openchat` directories, so it **can override** their settings.
...
Drop an `openchat.json` or `openchat.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/openchat/` |
| Linux    | `/etc/openchat/`                         |
| Windows  | `%ProgramData%\openchat`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, OpenCode reads managed preferences from the `ai.openchat.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.openchat.managed.plist`
2. `/Library/Managed Preferences/ai.openchat.managed.plist`

The plist keys map directly to `openchat.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.openchat.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.openchat.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.openchat.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.openchat</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`openchat.ai/config.json`**](https://openchat.ai/config.json).

TUI config uses [**`openchat.ai/tui.json`**](https://openchat.ai/tui.json).
...
Use `OPENCHAT_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `openchat.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `openchat serve` and `openchat web` commands through the `server` option.
...
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `openchat.local`. Useful for running multiple instances on the same network.
...
You can also define agents using markdown files in `~/.config/openchat/agents/` or `.openchat/agents/`. [Learn more here](/docs/agents).
...
This setting applies across all interfaces: TUI, CLI (`openchat run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/openchat/commands/` or `.openchat/commands/`. [Learn more here](/docs/commands).
...
By default, openchat **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
Place plugin files in `.openchat/plugins/` or `~/.config/openchat/plugins/`. You can also load plugins from npm through the `plugin` option.
