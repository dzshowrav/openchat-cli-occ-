...
:::caution
Wenn `OPENCHAT_SERVER_PASSWORD` nicht gesetzt ist, ist der Server ungesichert. Dies ist für den lokalen Gebrauch in Ordnung, sollte aber für den Netzwerkzugriff eingestellt werden.
:::

:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `openchat web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Dadurch wird der Hostname automatisch auf `0.0.0.0` gesetzt und der Server als `openchat.local` angekündigt.
...
Um den Zugriff zu schützen, legen Sie mithilfe der Umgebungsvariablen `OPENCHAT_SERVER_PASSWORD` ein Passwort fest:

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

Der Benutzername ist standardmäßig `openchat`, kann aber mit `OPENCHAT_SERVER_USERNAME` geändert werden.
...
Sie können Servereinstellungen auch in Ihrer `openchat.json`-Konfigurationsdatei konfigurieren:
