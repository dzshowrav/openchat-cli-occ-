...
Wenn Ihre globale Konfiguration beispielsweise `theme: "openchat"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/openchat`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/openchat/openchat.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENCHAT_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`openchat.json` im Projekt) – projektspezifische Einstellungen
5. **`.openchat` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`OPENCHAT_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.openchat` und `~/.config/openchat` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/openchat` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/openchat/openchat.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `openchat.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `OPENCHAT_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `OPENCHAT_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.openchat` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.openchat` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`openchat.ai/config.json`**](https://openchat.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `openchat serve` und `openchat web` über die Option `server` konfigurieren.
...
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `openchat.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/openchat/agents/` oder `.openchat/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`openchat run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/openchat/commands/` oder `.openchat/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.openchat/plugins/` oder `~/.config/openchat/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
