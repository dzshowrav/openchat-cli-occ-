...
- `.openchat/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openchat/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openchat/openchat.json`)
2. Projektkonfiguration (`openchat.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openchat/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openchat/plugins/`)
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://openchat.ai/docs/sdk).
