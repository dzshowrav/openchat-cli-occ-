---
title: Regeln
description: Lege eigene Anweisungen fuer openchat fest.
---

Du kannst openchat ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openchat den Befehl `/init` aus.
...
So kann openchat sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openchat sie zu erweitern.
...
openchat kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openchat/AGENTS.md` speichern.
Diese gelten in allen openchat-Sitzungen.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openchat/AGENTS.md` existiert)
...
Beim Start sucht openchat in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openchat/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openchat.json` oder in `~/.config/openchat/openchat.json` angeben.
...
openchat parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openchat.json

Empfohlen ist das `instructions`-Feld in der `openchat.json`:
...
Du kannst openchat explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openchat.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
