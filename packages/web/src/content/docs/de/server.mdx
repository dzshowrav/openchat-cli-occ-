---
title: Server
description: Nutze den openchat-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `openchat serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein openchat-Client nutzen kann.
...
Setze `OPENCHAT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `openchat`, kann aber mit `OPENCHAT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openchat serve` und `openchat web`.
...
Wenn du `openchat` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den openchat-Server, um openchat programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openchat serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openchat serve` trotzdem eine neue Serverinstanz.
...
Der openchat-Server stellt folgende APIs bereit.
