...
:::caution
Wenn du `--hostname 0.0.0.0` verwendest, setze `OPENCHAT_SERVER_PASSWORD`, um den Server zu schuetzen.
...
1. **Fuehre `openchat web` im WSL-Terminal aus**, nicht in PowerShell:
...
Wenn du `openchat web` aus WSL startest, bleiben Dateizugriff und Terminal-Integration sauber.
...
- OpenCode-Konfiguration und Sitzungen liegen in WSL unter `~/.local/share/openchat/`
