...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openchat\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openchat --log-level DEBUG`.

---

## Speicher

openchat speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/openchat/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop startet im Hintergrund einen lokalen OpenCode-Server (`openchat-cli`-Sidecar).
...
- **macOS/Linux**: `~/.config/openchat/openchat.jsonc` (oder `~/.config/openchat/openchat.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Globale Plugins**
  ...
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openchat\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/openchat`
- **Linux**: loesche `~/.cache/openchat` (oder fuehre aus: `rm -rf ~/.cache/openchat`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\openchat`
...
Wenn deine `openchat.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OPENCHAT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENCHAT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
- `openchat.global.dat` und `openchat.workspace.*.dat` (UI state like recent servers/projects)
...
3. Stelle sicher, dass du die neueste Version hast: `openchat upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `openchat models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\openchat`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. openchat installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\openchat`

2. Starte openchat neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt openchat, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
openchat erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
