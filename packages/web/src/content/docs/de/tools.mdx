...
:::note
Dieses Tool ist nur verfuegbar, wenn `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (oder `OPENCHAT_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenCode-Provider genutzt wird oder die Umgebungsvariable `OPENCHAT_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.
