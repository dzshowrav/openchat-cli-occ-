...
OpenCode lässt sich in VS Code, Cursor oder jede IDE integrieren, die ein Terminal unterstützt. Führen Sie einfach `openchat` im Terminal aus, um zu beginnen.
...
3. Führen Sie `openchat` aus – die Erweiterung wird automatisch installiert
...
- Stellen Sie sicher, dass Sie `openchat` im integrierten Terminal ausführen.
