---
title: Server
description: Interact with openchat server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openchat serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openchat client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openchat.local` |
...
Set `OPENCHAT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openchat`, or set `OPENCHAT_SERVER_USERNAME` to override it. This applies to both `openchat serve` and `openchat web`.
...
When you run `openchat` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openchat server to interact with openchat programmatically.
:::

This architecture lets openchat support multiple clients and allows you to interact with openchat programmatically.

You can run `openchat serve` to start a standalone server. If you have the
openchat TUI running, `openchat serve` will start a new server.
...
The openchat server exposes the following APIs.
