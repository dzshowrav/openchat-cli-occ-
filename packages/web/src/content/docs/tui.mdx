...
This is separate from `openchat.json`, which configures server/runtime behavior.
...
Use `OPENCHAT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `openchat.default`.
