---
title: Konfigurasjon
description: Ved å bruke openchat JSON-konfigurasjonen.
---
...
For eksempel, hvis din globale konfigurasjon setter `theme: "openchat"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/openchat`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/openchat/openchat.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCHAT_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`openchat.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.openchat` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`OPENCHAT_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.openchat` og `~/.config/openchat` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/openchat`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/openchat/openchat.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `openchat.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `OPENCHAT_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `OPENCHAT_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.openchat` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.openchat`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`openchat.ai/config.json`**](https://openchat.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `openchat serve` og `openchat web` gjennom alternativet `server`.
...
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `openchat.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/openchat/agents/` eller `.openchat/agents/`. [Les mer her](/docs/agents).
...
Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`openchat run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/openchat/commands/` eller `.openchat/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.openchat/plugins/` eller `~/.config/openchat/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
