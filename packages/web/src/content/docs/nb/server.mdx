...
Kommandoen `openchat serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openchat.local` |
...
Sett `OPENCHAT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openchat` som standard, eller sett `OPENCHAT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openchat serve` og `openchat web`.
...
Når du kjører `openchat` starter den en TUI og en server. Der TUI er
...
Du kan kjøre `openchat serve` for å starte en frittstående server. Hvis du har
OpenCode TUI kjører, vil `openchat serve` starte en ny server.
...
openchat-serveren viser følgende APIer.
