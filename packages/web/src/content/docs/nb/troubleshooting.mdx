...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\openchat\log`

Loggfiler navngis med tidsstempler (f.eks. `2025-01-09T123456.log`) og de siste 10 loggfilene beholdes.

Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `openchat --log-level DEBUG`.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\openchat`
...
OpenCode Desktop kjører en lokal OpenCode-server (`openchat-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.
...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/openchat/openchat.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\openchat\openchat.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\openchat\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/openchat`
- **Linux**: slett `~/.cache/openchat` (eller kjør `rm -rf ~/.cache/openchat`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\openchat`
...
Hvis `openchat.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `OPENCHAT_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `OPENCHAT_PORT` (eller velg en ledig port) og start på nytt.
...
- `openchat.settings.dat` (stasjonær standardserver URL)
- `openchat.global.dat` og `openchat.workspace.*.dat` (UI tilstand som nylige servere/prosjekter)
...
1. Tøm leverandørens pakkebuffer:
   ...
   På Windows, trykk `WIN+R` og slett: `%USERPROFILE%\.cache\openchat`
