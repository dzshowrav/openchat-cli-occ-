...
:::note
Dette verktøyet er bare tilgjengelig når `OPENCHAT_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENCHAT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `OPENCHAT_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
