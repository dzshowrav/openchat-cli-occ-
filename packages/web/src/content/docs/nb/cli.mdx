...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Økt ID for å fortsette                                                                             |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren økten ved fortsettelse (bruk med `--continue` eller `--session`)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCHAT_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCHAT_SERVER_USERNAME` eller `openchat`) |
...
OpenCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `openchat auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/openchat/auth.json`.
...
Du kan også koble til en kjørende `openchat serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
Dette starter en HTTP-server som gir API tilgang til OpenCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCHAT_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `openchat`).
...
Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenCode via et nettgrensesnitt. Sett `OPENCHAT_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `openchat`).
...
| ------------------------------------- | ------ | --------------------------------------------------------------------- |
| `OPENCHAT_AUTO_SHARE`                 | boolsk | Del økter automatisk                                                  |
| `OPENCHAT_GIT_BASH_PATH`              | streng | Bane til Git Bash-kjørbar på Windows                                  |
| `OPENCHAT_CONFIG`                     | streng | Bane til konfigurasjonsfil                                            |
| `OPENCHAT_TUI_CONFIG`                 | streng | Bane til TUI-konfigurasjonsfil                                        |
| `OPENCHAT_CONFIG_DIR`                 | streng | Bane til konfigurasjonskatalog                                        |
| `OPENCHAT_CONFIG_CONTENT`             | streng | Innebygd json-konfigurasjonsinnhold                                   |
| `OPENCHAT_DISABLE_AUTOUPDATE`         | boolsk | Deaktiver automatiske oppdateringskontroller                          |
| `OPENCHAT_DISABLE_PRUNE`              | boolsk | Deaktiver beskjæring av gamle data                                    |
| `OPENCHAT_DISABLE_TERMINAL_TITLE`     | boolsk | Deaktiver automatiske terminaltitteloppdateringer                     |
| `OPENCHAT_PERMISSION`                 | streng | Innebygd json-tillatelseskonfigurasjon                                |
| `OPENCHAT_DISABLE_DEFAULT_PLUGINS`    | boolsk | Deaktiver standard plugins                                            |
| `OPENCHAT_DISABLE_LSP_DOWNLOAD`       | boolsk | Deaktiver automatiske LSP servernedlastinger                          |
| `OPENCHAT_ENABLE_EXPERIMENTAL_MODELS` | boolsk | Aktiver eksperimentelle modeller                                      |
| `OPENCHAT_DISABLE_AUTOCOMPACT`        | boolsk | Deaktiver automatisk kontekstkomprimering                             |
| `OPENCHAT_DISABLE_CLAUDE_CODE`        | boolsk | Deaktiver lesing fra `.claude` (spørsmål + ferdigheter)               |
| `OPENCHAT_DISABLE_CLAUDE_CODE_PROMPT` | boolsk | Deaktiver lesing `~/.claude/CLAUDE.md`                                |
| `OPENCHAT_DISABLE_CLAUDE_CODE_SKILLS` | boolsk | Deaktiver innlasting av `.claude/skills`                              |
| `OPENCHAT_DISABLE_MODELS_FETCH`       | boolsk | Deaktiver henting av modeller fra eksterne kilder                     |
| `OPENCHAT_FAKE_VCS`                   | streng | Falsk VCS-leverandør for testformål                                   |
| `OPENCHAT_CLIENT`                     | streng | Klientidentifikator (standard til `cli`)                              |
| `OPENCHAT_ENABLE_EXA`                 | boolsk | Aktiver Exa-nettsøkeverktøy                                           |
| `OPENCHAT_SERVER_PASSWORD`            | streng | Aktiver grunnleggende autentisering for `serve`/`web`                 |
| `OPENCHAT_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `openchat`) |
| `OPENCHAT_MODELS_URL`                 | streng | Egendefinert URL for henting av modellkonfigurasjon                   |
...
| Variabel                                        | Type   | Beskrivelse                                           |
| ----------------------------------------------- | ------ | ----------------------------------------------------- |
| `OPENCHAT_EXPERIMENTAL`                         | boolsk | Aktiver eksperimentelle funksjoner under samleflagget |
| `OPENCHAT_EXPERIMENTAL_ICON_DISCOVERY`          | boolsk | Aktiver ikonoppdagelse                                |
| `OPENCHAT_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolsk | Deaktiver kopi ved valg i TUI                         |
| `OPENCHAT_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | tall   | Standard tidsavbrudd for bash-kommandoer i ms         |
| `OPENCHAT_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | tall   | Maks Output Tokens for LLM-svar                       |
| `OPENCHAT_EXPERIMENTAL_FILEWATCHER`             | boolsk | Aktiver filovervåker for hele dir                     |
| `OPENCHAT_EXPERIMENTAL_OXFMT`                   | boolsk | Aktiver oxfmt formatter                               |
| `OPENCHAT_EXPERIMENTAL_LSP_TOOL`                | boolsk | Aktiver eksperimentelt LSP-verktøy                    |
| `OPENCHAT_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolsk | Deaktiver filovervåking                               |
| `OPENCHAT_EXPERIMENTAL_EXA`                     | boolsk | Aktiver eksperimentelle Exa-funksjoner                |
| `OPENCHAT_EXPERIMENTAL_LSP_TY`                  | boolsk | Aktiver TY LSP for python-filer                       |
| `OPENCHAT_EXPERIMENTAL_PLAN_MODE`               | boolsk | Aktiver planmodus                                     |
| `OPENCHAT_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolsk | Aktiver bakgrunnsoppgaver for subagenter              |
| `OPENCHAT_EXPERIMENTAL_EVENT_SYSTEM`            | boolsk | Aktiver eksperimentelt hendelsessystem                |
| `OPENCHAT_EXPERIMENTAL_NATIVE_LLM`              | boolsk | Aktiver innebygd LLM-forespørselsvei                  |
| `OPENCHAT_EXPERIMENTAL_PARALLEL`                | boolsk | Aktiver parallell kjøring av websøk                   |
| `OPENCHAT_EXPERIMENTAL_SCOUT`                   | boolsk | Aktiver Scout-subagent                                |
| `OPENCHAT_EXPERIMENTAL_WORKSPACES`              | boolsk | Aktiver arbeidsområde-støtte                          |
