---
title: GitHub
description: Bruk openchat i GitHub-problemer og pull-forespørsler.
---

OpenCode integreres med GitHub-arbeidsflyten din. Nevn `/openchat` eller `/oc` i kommentaren din, og OpenCode vil utføre oppgaver i din GitHub Actions-løper.
...
1. **Installer GitHub-appen**
   ...
   Gå over til [**github.com/apps/openchat-agent**](https://github.com/apps/openchat-agent). Sørg for at den er installert på mållageret.

2. **Legg til arbeidsflyten**

   Legg til følgende arbeidsflytfil til `.github/workflows/openchat.yml` i repoen. Sørg for å sette riktig `model` og nødvendige API-nøkler i `env`.
...
| ----------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentar på sak eller PR               | Nevn `/openchat` eller `/oc` i kommentaren din. OpenCode leser kontekst og kan opprette grener, åpne PR-er eller svare. |
| `pull_request_review_comment` | Kommenter spesifikke kodelinjer i en PR | Nevn `/openchat` eller `/oc` mens du ser gjennom koden. OpenCode mottar filbane, linjenummer og diff-kontekst.          |
...
:::tip
Du kan konfigurere til å bruke en annen triggerfrase enn `@openchat`.
:::
