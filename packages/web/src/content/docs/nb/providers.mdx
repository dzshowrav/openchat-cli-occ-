...
i `~/.local/share/openchat/auth.json`.
...
1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Zen` og gå til [openchat.ai/auth](https://openchat.ai/zen).
...
1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Go`, og gå til [openchat.ai/auth](https://openchat.ai/zen).
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openchat.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openchat støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openchat til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
`openchat.json` fil. Det anbefales også å deaktivere øktdeling.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openchat-helicone-session](https://github.com/H2Shami/openchat-helicone-session) plugin for automatisk å logge hver OpenCode-samtale som en økt i Helicone.
...
:::tip
Ollama kan automatisk konfigurere seg selv for OpenCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openchat) for detaljer.
:::
...
4. Opprett eller oppdater `openchat.json`-filen i prosjektkatalogen:
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openchat auth list` for å se om legitimasjonen
