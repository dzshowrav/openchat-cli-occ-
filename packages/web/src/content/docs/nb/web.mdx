...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `OPENCHAT_SERVER_PASSWORD`:

```bash
OPENCHAT_SERVER_PASSWORD=secret openchat web
```

Brukernavnet er satt til `openchat` som standard, men kan endres med `OPENCHAT_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `openchat.json` konfigurasjonsfilen:
