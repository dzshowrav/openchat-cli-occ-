...
openchat søker etter disse stedene:

- Prosjektkonfigurasjon: `.openchat/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openchat/skills/<name>/SKILL.md`
...
For prosjektlokale stier går openchat opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openchat/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openchat/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openchat/skills/git-release/SKILL.md` slik:
...
openchat viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `openchat.json`:
...
**For innebygde agenter** (i `openchat.json`):
