...
OpenCode integrates with VS Code, Cursor, or any IDE that supports a terminal. Just run `openchat` in the terminal to get started.
...
3. Run `openchat` - the extension installs automatically
...
- Ensure you’re running `openchat` in the integrated terminal.
