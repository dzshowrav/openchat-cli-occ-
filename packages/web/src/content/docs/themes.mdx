...
By default, OpenCode uses our own `openchat` theme.
...
2. **User config directory** - Defined in `~/.config/openchat/themes/*.json` or `$XDG_CONFIG_HOME/openchat/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openchat/themes/*.json`
4. **Current working directory** - Defined in `./.openchat/themes/*.json`
