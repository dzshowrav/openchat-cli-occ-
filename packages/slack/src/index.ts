...
import { createOpenchat, type ToolPart } from "@openchat-ai/sdk"
...
console.log("🚀 Starting openchat server...")
const openchat = await createOpenchat({
  port: 0,
})
console.log("✅ Openchat server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await openchat.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openchat session...")
    const { client, server } = openchat
    ...
    console.log("✅ Created openchat session:", createResult.data.id)
  ...
  console.log("📝 Sending to openchat:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Openchat response:", JSON.stringify(result, null, 2))
