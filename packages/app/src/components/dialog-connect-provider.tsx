import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@openchat-ai/sdk/v2/client"
import { Button } from "@openchat-ai/ui/button"
import { useDialog } from "@openchat-ai/ui/context/dialog"
import { Dialog } from "@openchat-ai/ui/dialog"
import { Icon } from "@openchat-ai/ui/icon"
import { IconButton } from "@openchat-ai/ui/icon-button"
import { List, type ListRef } from "@openchat-ai/ui/list"
import { ProviderIcon } from "@openchat-ai/ui/provider-icon"
import { Spinner } from "@openchat-ai/ui/spinner"
import { Tag } from "@openchat-ai/ui/tag"
import { TextField } from "@openchat-ai/ui/text-field"
import { ButtonV2 } from "@openchat-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@openchat-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@openchat-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "openchat-go") return language.t("dialog.provider.openchatGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "openchat"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.openchat.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["openchat", "openchat-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "openchat" || provider.id === "openchat-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "openchat"
                                ? "dialog.provider.openchat.tagline"
                                : "dialog.provider.openchatGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "openchat"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.openchatZen.line1")}</div>
              <div>{language.t("provider.connect.openchatZen.line2")}</div>
              <div>
                {language.t("provider.connect.openchatZen.visit.prefix")}
                <Link
                  href="https://openchat.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.openchatZen.visit.link")}
                </Link>
                {language.t("provider.connect.openchatZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "openchat"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.openchatZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.openchatZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.openchatZen.visit.prefix")}
                <Link href="https://openchat.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.openchatZen.visit.link")}
                </Link>
                {language.t("provider.connect.openchatZen.visit.suffix")}
              </div>
