  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://openchat.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://openchat.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://openchat.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://openchat.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://openchat.example.com"),
      ),
    ).toEqual({
