...
import type { SessionNotFoundError } from "@openchat-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em openchat.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
