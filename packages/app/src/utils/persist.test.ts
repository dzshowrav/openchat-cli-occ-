...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("openchat.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("openchat.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("openchat.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("openchat.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("openchat.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("openchat.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("openchat.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("openchat.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("openchat.safe.scope:value")).toBe('{"value":3}')
  })

  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("openchat.throw.scope2")
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("openchat.workspace.")
