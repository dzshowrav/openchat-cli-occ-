...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when OpenCode needs attention", () => {
    expect(wslOpenchatAction(undefined)).toBeUndefined()
    expect(
      wslOpenchatAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    expect(
      wslOpenchatAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/openchat",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when OpenCode is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...
        openchatChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.openchat/bin/openchat",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "openchat is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installOpenchat" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-openchat")
  })
