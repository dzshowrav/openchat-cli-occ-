...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-openchat" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOpenchatAction(check?: WslOpenchatCheck) {
  if (!check) return
...
export function addServerViewModel(input: {
  ...
  const openchatCheck = selectedDistro ? (state?.openchatChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.openchatChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateOpenchat" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.openchatMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installOpenchat" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  openchatCheck: WslOpenchatCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpenchat =
    input.state?.job?.kind === "install-openchat" && input.state.job.distro === input.selectedDistro
  if (!ready || probingOpenchat) {
    return {
      variant: "contrast",
  ...
  if (!addServerOpenchatReady(input.openchatCheck)) {
    const update = !!input.openchatCheck?.resolvedPath && input.openchatCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingOpenchat
        ? { key: "wsl.onboarding.updatingOpenchat" }
        : update
          ? { key: "wsl.onboarding.updateOpenchat" }
          : { key: "wsl.onboarding.installOpenchat" },
      disabled: !!input.state?.job || input.adding,
      action: "install-openchat",
      loading: installingOpenchat,
...
function addServerOpenchatReady(check: WslOpenchatCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.openchatChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.openchatChecks[item.name]) return [`openchat:${item.name}`]
    return []
  })
