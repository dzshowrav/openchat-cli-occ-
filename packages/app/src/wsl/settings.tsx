...
import { wslOpenchatAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.openchatChecks[item.config.distro]
          const openchatAction = () => wslOpenchatAction(check())
          const busy = () => wsl.data?.job?.kind === "install-openchat" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={openchatAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOpenchat(item.config.distro))}
                    >
