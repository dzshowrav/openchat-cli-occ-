...
import { type Session } from "@openchat-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openchat://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openchat://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openchat://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openchat://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openchat://open-project")).toBeUndefined()
    expect(parseDeepLink("openchat://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openchat://open-project?directory=/a",
      "openchat://other?directory=/b",
      "openchat://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openchat://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openchat://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openchat://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openchat://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openchat://new-session?directory=/a",
      "openchat://open-project?directory=/b",
      "openchat://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCHAT__: {
        deepLinks: ["openchat://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCHAT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openchat://open-project?directory=/a"])
