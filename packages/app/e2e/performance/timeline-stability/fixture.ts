...
import { SessionStatusEvent } from "@openchat-ai/schema/session-status-event"
import { SessionV1 } from "@openchat-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const model = { providerID: "openchat", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("openchat.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["openchat"],
    default: { providerID: "openchat", modelID: "claude-opus-4-6" },
  }
}
