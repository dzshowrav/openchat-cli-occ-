...
import { lazy } from "@openchat-ai/core/util/lazy"
...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.OPENCHAT_STORAGE_BUCKET!
    const region = process.env.OPENCHAT_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.OPENCHAT_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.OPENCHAT_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.OPENCHAT_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.OPENCHAT_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.OPENCHAT_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://${accountId}.r2.cloudflarestorage.com`, process.env.OPENCHAT_STORAGE_BUCKET!)
  }

  const adapter = lazy(() => {
    const type = process.env.OPENCHAT_STORAGE_ADAPTER
