...
- Running `bun dev` from `packages/openchat` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s openchat-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt openchat-dev`.
- Stop the session explicitly when done: `tmux kill-session -t openchat-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@openchat/Foo") {}
