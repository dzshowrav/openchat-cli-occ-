...
it.instance("plan agent denies edits except .openchat/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openchat/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".openchat", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENCHAT_TEST_HOME
      process.env.OPENCHAT_TEST_HOME = test.directory
