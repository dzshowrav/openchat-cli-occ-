...
import type { Event, OpenchatClient } from "@openchat-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<OpenchatClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "openchat-acp-permission-"))
