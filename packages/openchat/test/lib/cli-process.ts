// Subprocess test harness for the openchat CLI. Spawns the real binary against
...
//
// Configuration flows through openchat's built-in test affordances:
//   - OPENCHAT_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCHAT_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCHAT_DISABLE_PROJECT_CONFIG : skip walking up for openchat.json
//   - OPENCHAT_PURE                : skip external plugin discovery + install
//   - OPENCHAT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `openchat.run` is fully wired. The shape supports adding more
// builders (`openchat.serve(opts)`, `openchat.acp(opts)`, `openchat.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpenchatCli.
...
import { AppProcess } from "@openchat-ai/core/process"
...
const openchatRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(openchatRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENCHAT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENCHAT_CONFIG_CONTENT: configJson,
    OPENCHAT_DISABLE_PROJECT_CONFIG: "1",
    OPENCHAT_PURE: "1",
    OPENCHAT_DISABLE_AUTOUPDATE: "1",
    OPENCHAT_DISABLE_AUTOCOMPACT: "1",
    OPENCHAT_DISABLE_MODELS_FETCH: "1",
    OPENCHAT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `openchat run`. New flags should
...
// `openchat serve` is a long-lived process — it never exits on its own.
...
// `openchat acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpenchatCli = {
  ...
  // Spawn `openchat serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `openchat acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("openchat.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OPENCHAT_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("openchat.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("openchat.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "openchat server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `openchat serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("openchat.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const openchat: OpenchatCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, openchat })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `openchat.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "openchat") {
...
// every call site. Use this for any test that needs the openchat CLI fixture.
//
...
// resources (e.g. `openchat.serve`) without an extra `Effect.scoped` wrapper —
