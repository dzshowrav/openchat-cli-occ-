...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.OPENCHAT_WEBSEARCH_PROVIDER

    try {
      process.env.OPENCHAT_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.OPENCHAT_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.OPENCHAT_WEBSEARCH_PROVIDER
      else process.env.OPENCHAT_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for openchat or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.openchat, { exa: false, parallel: false })).toBe(true)
