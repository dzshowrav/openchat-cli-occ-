...
import { EventSequenceTable } from "@openchat-ai/core/event/sql"
...
import { WorkspaceV2 } from "@openchat-ai/core/workspace"
import { WorkspaceTable } from "@openchat-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  OPENCHAT_AUTH_CONTENT: process.env.OPENCHAT_AUTH_CONTENT,
  OPENCHAT_EXPERIMENTAL_WORKSPACES: process.env.OPENCHAT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OPENCHAT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@openchat.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENCHAT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=openchat-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OPENCHAT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OPENCHAT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OPENCHAT_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=openchat-test")
