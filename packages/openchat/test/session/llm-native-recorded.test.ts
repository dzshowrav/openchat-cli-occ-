...
import { SessionV1 } from "@openchat-ai/core/v1/session"
import { ModelsDev } from "@openchat-ai/core/models-dev"
import { HttpRecorder } from "@openchat-ai/http-recorder"
import { HttpRecorderInternal } from "@openchat-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@openchat-ai/llm"
import { RequestExecutor } from "@openchat-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.openchat.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OPENCHAT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["openchat", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENCHAT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OPENCHAT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["openchat", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "openchat-proxy",
    name: "OpenCode proxy",
    providerID: ProviderV2.ID.openchat,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["openchat", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OPENCHAT_RECORD_CONSOLE_TOKEN && process.env.OPENCHAT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openchat,
        name: "OpenCode Zen",
        env: ["OPENCHAT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OPENCHAT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OPENCHAT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OPENCHAT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["openchat", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENCHAT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OPENCHAT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OPENCHAT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OPENCHAT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the openchat LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "openchat.json"),
      JSON.stringify({ $schema: "https://openchat.ai/config.json", ...scenario.config(model) }),
    ),
  )
