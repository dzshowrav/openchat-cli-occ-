...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ openchat }) =>
    Effect.gen(function* () {
      const result = yield* openchat.spawn(["--replay-limit", "10"])

      openchat.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ openchat }) =>
    Effect.gen(function* () {
      const result = yield* openchat.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ openchat }) =>
    Effect.gen(function* () {
      const result = yield* openchat.spawn(["--mini", "--port", "4096"])
