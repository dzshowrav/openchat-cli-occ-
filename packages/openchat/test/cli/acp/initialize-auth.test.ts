...
describe("openchat acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ openchat }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ openchat }))
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ openchat })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("openchat-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "openchat-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OPENCHAT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("openchat-login")
