...
describe("openchat read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OPENCHAT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["mcp", "list"])
        openchat.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["providers", "list"])
        openchat.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["models"])
        openchat.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["agent", "list"])
        openchat.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OPENCHAT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["session", "list"])
        openchat.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["stats"])
        openchat.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OPENCHAT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ openchat }) =>
      Effect.gen(function* () {
        const r = yield* openchat.spawn(["db", "path"])
        openchat.expectExit(r, 0, "db path")
