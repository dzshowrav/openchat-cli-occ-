...
// terminal sizes. The default openchat tui command is excluded —
// `openchat --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `openchat --help` prints today;
...
// top-level help on `--help` and exits 1; not a real openchat command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("openchat CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const topLevel = yield* openchat.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* openchat.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`openchat ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`openchat ${argv.join(" ")} --help`)
        }
