// Subprocess integration tests for `openchat run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `openchat.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OPENCHAT_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("openchat run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openchat.run("say hi")
        openchat.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openchat.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* openchat.run("think", { extraArgs: ["--thinking"] })
        openchat.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* openchat.run("think again")
        openchat.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openchat.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openchat.run("say hi", { format: "json" })
        openchat.expectExit(result, 0)

        const events = openchat.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ openchat }) =>
      Effect.gen(function* () {
        const result = yield* openchat.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openchat.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const result = yield* openchat.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* openchat.run("request permission", { permission: { bash: "ask" } })
        openchat.expectExit(denied, 0)
        ...
        const allowed = yield* openchat.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        openchat.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* openchat.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        openchat.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, openchat }) =>
      Effect.gen(function* () {
        ...
        const server = yield* openchat.serve()

        const result = yield* openchat.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, openchat }) =>
      Effect.gen(function* () {
        const result = yield* openchat.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, openchat }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* openchat.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
