...
import { OpenchatClient, type GlobalEvent } from "@openchat-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OpenchatClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OpenchatClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OpenchatClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OpenchatClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OpenchatClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://openchat.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OpenchatClient["event"]["subscribe"]
    globalEvent?: OpenchatClient["global"]["event"]
    promptAsync?: OpenchatClient["session"]["promptAsync"]
    status?: OpenchatClient["session"]["status"]
    messages?: OpenchatClient["session"]["messages"]
    children?: OpenchatClient["session"]["children"]
    permissions?: OpenchatClient["permission"]["list"]
    questions?: OpenchatClient["question"]["list"]
  } = {},
) {
  const client = new OpenchatClient()

  const subscribe: OpenchatClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OpenchatClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OpenchatClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OpenchatClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OpenchatClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OpenchatClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OpenchatClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OpenchatClient["question"]["list"] = input.questions ?? (() => ok([]))
