...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-openchat-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-openchat-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://openchat.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openchat-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-openchat-workspace")
