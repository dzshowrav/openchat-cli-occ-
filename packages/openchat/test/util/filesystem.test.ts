...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openchat.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openchat.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openchat.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openchat.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openchat.json", "openchat.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openchat.json"),
        path.join(project, "openchat.jsonc"),
      ])
    })
  })
