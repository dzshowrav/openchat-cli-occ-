...
Practical reference for a future `packages/server` split after the openchat
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/openchat`.
...
Until enough shared service code lives outside `packages/openchat`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openchat`

It should not import `packages/openchat` services while `packages/openchat`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on openchat-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openchat` implementation details.
