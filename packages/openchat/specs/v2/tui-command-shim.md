...
Delete `packages/openchat/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openchat/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openchat/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/openchat`
- TUI plugin loader tests in `packages/openchat` if runtime plugin API wiring changed
