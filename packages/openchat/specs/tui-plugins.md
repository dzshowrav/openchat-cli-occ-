...
- Author package entrypoint is `@openchat-ai/plugin/tui`.
...
```json
{
  "$schema": "https://openchat.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/openchat-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@openchat-ai/plugin/tui`.
- `@openchat-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openchat-ai/plugin/tui"
...
- If `engines.openchat` is absent, no check is performed (backward compatible).
...
- `openchat plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `openchat plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.openchat` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openchat`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openchat`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openchat` dir or `OPENCHAT_CONFIG_DIR`, OpenCode installs `@openchat-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openchat-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openchat-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"openchat"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `openchat.default`.
...
- Local plugins persist installed themes under the local `.openchat/themes` area near the plugin config source.
...
- `--pure` / `OPENCHAT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openchat/plugins/tui-smoke.tsx`
- Local vim plugin: `.openchat/plugins/tui-vim.tsx`
- Local smoke config: `.openchat/tui.json`
- Local smoke theme: `.openchat/plugins/smoke-theme.json`
