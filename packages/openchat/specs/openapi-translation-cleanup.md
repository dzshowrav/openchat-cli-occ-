...
Trim `packages/openchat/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/openchat/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/openchat`.
- `[x]` `bun typecheck` from `packages/openchat`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/openchat/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/openchat`.
- `[x]` `bun dev generate > /tmp/openchat-openapi.json` from `packages/openchat`.
...
- Edit `packages/openchat/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/openchat-openapi.json` from `packages/openchat`.
...
- Check source schemas in files like `packages/openchat/src/session/schema.ts`, `packages/openchat/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/openchat/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/openchat/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
