...
const MANAGED_PLIST_DOMAIN = "ai.openchat.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/openchat"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "openchat")
    default:
      return "/etc/openchat"
  }
}
...
export function managedConfigDir() {
  return process.env.OPENCHAT_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
