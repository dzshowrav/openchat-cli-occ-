...
const TUI_SCHEMA_URL = "https://openchat.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openchat.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openchat = await openchatFiles(input)
  for (const file of openchat) {
...
async function openchatFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "openchat"),
    ...(await Filesystem.findUp(["openchat.json", "openchat.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openchat"))
  }
  if (Flag.OPENCHAT_CONFIG) files.push(Flag.OPENCHAT_CONFIG)
