...
import { isRecord } from "@openchat-ai/tui/util/record"
...
import { TuiKeybind } from "@openchat-ai/tui/config/keybind"
...
import { TuiConfig } from "@openchat-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@openchat/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old openchat.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.openchat`
  // folders between cwd and home, and OPENCHAT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OPENCHAT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPENCHAT_TUI_CONFIG override, if set.
  if (Flag.OPENCHAT_TUI_CONFIG) {
    const configFile = Flag.OPENCHAT_TUI_CONFIG
  ...
  // 4. `.openchat` directories (and OPENCHAT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".openchat") || dir === Flag.OPENCHAT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".openchat") && dir !== Flag.OPENCHAT_CONFIG_DIR) continue
