...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@openchat-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@openchat-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@openchat-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@openchat-ai/tui/context/theme"
import { Global } from "@openchat-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@openchat-ai/core/util/flock"
import { Flag } from "@openchat-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@openchat-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@openchat-ai/core/v1/config/plugin"
import { createCommandShim } from "@openchat-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@openchat-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openchat"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openchat", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openchat", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OPENCHAT_PURE ? [] : pluginOrigins
    if (Flag.OPENCHAT_PURE && pluginOrigins.length) {
    }
