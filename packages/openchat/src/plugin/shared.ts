...
export const DEPRECATED_PLUGIN_PACKAGES = ["openchat-openai-codex-auth", "openchat-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openchatVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openchatVersion) || semver.major(openchatVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openchat
  if (typeof range !== "string") return
  if (!semver.satisfies(openchatVersion, range)) {
    throw new Error(`Plugin requires openchat ${range} but running ${openchatVersion}`)
  }
}
