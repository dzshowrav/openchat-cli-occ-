import type { Hooks, PluginInput } from "@openchat-ai/plugin"
import { OAUTH_DUMMY_KEY } from "../auth"
import { createServer } from "http"
import { InstallationVersion } from "@openchat-ai/core/installation/version"
import { OauthCallbackPage } from "@openchat-ai/core/oauth/page"
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=openchat` lets xAI attribute openchat-originated
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire openchat process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `openchat/${InstallationVersion}`)
