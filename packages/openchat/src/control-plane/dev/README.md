This is a plugin to simulate a remote environment locally. Add this to `.openchat/openchat.jsonc`:

```json
  "plugin": ["../packages/openchat/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/openchat/script/run-workspace-server
```
