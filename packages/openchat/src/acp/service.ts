...
import type { AssistantMessage, Message, OpenchatClient, SessionMessageResponse } from "@openchat-ai/sdk/v2"
...
export const AuthMethodID = "openchat-login"
...
export class Service extends Context.Service<Service, Interface>()("@openchat/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `openchat auth login` in the terminal",
      name: "Login with openchat",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OpenchatClient) {
...
function makeUsageService(sdk: OpenchatClient) {
...
async function loadDirectorySnapshot(sdk: OpenchatClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, openchat provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const openchatProvider = providers[ProviderV2.ID.make("openchat")]
  const openchatModel = openchatProvider ? Provider.sort(Object.values(openchatProvider.models))[0] : undefined
  if (openchatProvider && openchatModel) return { providerID: openchatProvider.id, modelID: openchatModel.id }
