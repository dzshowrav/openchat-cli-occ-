...
import type { LLMClientShape } from "@openchat-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("openchat"))
    return { type: "unsupported", reason: "provider is not openai, openchat, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @openchat-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains openchat-owned. The native runtime only adapts
      // the @openchat-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
