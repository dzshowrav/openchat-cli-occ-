...
import { SkillPlugin } from "@openchat-ai/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@openchat-ai/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@openchat-ai/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@openchat-ai/core/util/glob"
...
// Built-in skill that ships with openchat. The model's intuition for what an
// openchat.json should look like is often wrong, and openchat hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch openchat's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_OPENCHAT_SKILL_NAME = "customize-openchat"
const CUSTOMIZE_OPENCHAT_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating openchat's own configuration: openchat.json, openchat.jsonc, files under .openchat/, or files under ~/.config/openchat/. Also use when creating or fixing openchat agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring openchat itself."
const CUSTOMIZE_OPENCHAT_SKILL_BODY = SkillPlugin.CustomizeOpenchatContent
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_OPENCHAT_SKILL_NAME] = {
          name: CUSTOMIZE_OPENCHAT_SKILL_NAME,
          description: CUSTOMIZE_OPENCHAT_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_OPENCHAT_SKILL_BODY,
        }
