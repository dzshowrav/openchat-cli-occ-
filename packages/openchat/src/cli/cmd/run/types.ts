// Shared type vocabulary for the direct interactive mode (`openchat --mini`).
//
...
import type { OpenchatClient, PermissionRequest, QuestionRequest, ToolPart } from "@openchat-ai/sdk/v2"
import type { TuiConfig } from "@openchat-ai/tui/config"
...
type PromptModel = Parameters<OpenchatClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<OpenchatClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<OpenchatClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<OpenchatClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<OpenchatClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<OpenchatClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<OpenchatClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<OpenchatClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<OpenchatClient["question"]["reject"]>[0]
