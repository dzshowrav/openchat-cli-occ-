import type { PermissionV1 } from "@openchat-ai/core/v1/permission"
import { FSUtil } from "@openchat-ai/core/fs-util"
// CLI entry point for `openchat run` and `openchat --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`openchat --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`openchat --mini --attach`): connects to a running
//      openchat server and runs interactive mode against it.
//
...
import { createOpenchatClient, type OpenchatClient, type ToolPart } from "@openchat-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<OpenchatClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run openchat with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openchat server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENCHAT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCHAT_SERVER_USERNAME or 'openchat')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: OpenchatClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OpenchatClient, sessionID: string) {
      ...
      async function current(sdk: OpenchatClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OpenchatClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OpenchatClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OpenchatClient) {
        ...
        async function loop(client: OpenchatClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createOpenchatClient({
        baseUrl: "http://openchat.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
