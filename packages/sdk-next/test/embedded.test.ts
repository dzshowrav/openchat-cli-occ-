...
test("embedded client uses the real router and handlers", async () => {
  const directory = await mkdtemp(join(tmpdir(), "openchat-embedded-"))
  const database = Flag.OPENCHAT_DB
  Flag.OPENCHAT_DB = join(directory, "openchat.sqlite")
  ...
  try {
    const program = Effect.gen(function* () {
      const openchat = yield* OpenCode.create()
      yield* openchat.tools.register({
      ...
      const created = yield* openchat.sessions.create({
        id: sessionID,
        agent: Agent.ID.make("build"),
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      yield* openchat.sessions.switchModel({ sessionID, model })
      const selected = yield* openchat.sessions.get({ sessionID })
      const page = yield* openchat.sessions.list({ directory: AbsolutePath.make(directory) })
      const active = yield* openchat.sessions.active()
      const admitted = yield* openchat.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Do not run" }),
        resume: false,
      })
      const context = yield* openchat.sessions.context({ sessionID })
      const wake = yield* openchat.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Promote this input" }),
      })
      const prompted = yield* openchat.sessions.events({ sessionID }).pipe(
        Stream.filter((event) => event.type === "session.next.prompted" && event.data.messageID === wake.id),
        Stream.runHead,
        Effect.timeout("10 seconds"),
        Effect.map(Option.getOrThrow),
      )
      const wakeContext = yield* openchat.sessions.context({ sessionID })
      const event = yield* openchat.sessions
        .events({ sessionID })
        .pipe(Stream.take(1), Stream.runHead, Effect.map(Option.getOrUndefined))
      const modelMessage = Option.fromNullishOr(context.find((message) => message.type === "model-switched")).pipe(
        Option.getOrThrow,
      )
      const message = yield* openchat.sessions.message({ sessionID, messageID: modelMessage.id })
      yield* openchat.sessions.interrupt({ sessionID })
      const other = yield* openchat.sessions.create({
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      const missingSessionID = Session.ID.make(`ses_missing_${crypto.randomUUID()}`)
      const missing = yield* Effect.all(
        [
          openchat.sessions.events({ sessionID: missingSessionID }).pipe(Stream.runHead, Effect.flip),
          openchat.sessions.interrupt({ sessionID: missingSessionID }).pipe(Effect.flip),
          openchat.sessions.message({ sessionID: missingSessionID, messageID: modelMessage.id }).pipe(Effect.flip),
        ],
        { concurrency: "unbounded" },
      )
      const missingMessage = yield* Effect.flip(
        openchat.sessions.message({
          sessionID: other.id,
    ...
    Flag.OPENCHAT_DB = database
    await rm(directory, { recursive: true, force: true })
  }
})

test("Location-owned runner events reach the ready global client", async () => {
  const directory = await mkdtemp(join(tmpdir(), "openchat-embedded-events-"))
  const database = Flag.OPENCHAT_DB
  Flag.OPENCHAT_DB = join(directory, "openchat.sqlite")
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      yield* openchat.events.subscribe().pipe(
      ...
      yield* openchat.sessions.create({
        id: sessionID,
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      yield* openchat.sessions.prompt({ sessionID, prompt: Prompt.make({ text: "Observe this input" }) })
...
test("independent embedded hosts do not share live notifications", async () => {
  const directory = await mkdtemp(join(tmpdir(), "openchat-embedded-hosts-"))
  const database = Flag.OPENCHAT_DB
  Flag.OPENCHAT_DB = join(directory, "openchat.sqlite")
...
test("embedded client is available as a Layer service", async () => {
  const directory = await mkdtemp(join(tmpdir(), "openchat-embedded-layer-"))
  const database = Flag.OPENCHAT_DB
  Flag.OPENCHAT_DB = join(directory, "openchat.sqlite")
  ...
  try {
    const created = await Effect.runPromise(
      Effect.gen(function* () {
        const openchat = yield* OpenCode.Service
        return yield* openchat.sessions.create({
          id: sessionID,
          location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
        })
