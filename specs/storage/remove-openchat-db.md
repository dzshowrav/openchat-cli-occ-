# Remove `packages/openchat/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/openchat/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the openchat legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/openchat/src/storage/db.ts` are concentrated in 22 source files:

- `packages/openchat/src/account/repo.ts`
- `packages/openchat/src/cli/cmd/db.ts`
- `packages/openchat/src/cli/cmd/import.ts`
- `packages/openchat/src/cli/cmd/stats.ts`
- `packages/openchat/src/control-plane/workspace.ts`
- `packages/openchat/src/index.ts`
- `packages/openchat/src/node.ts`
- `packages/openchat/src/permission/index.ts`
- `packages/openchat/src/project/project.ts`
- `packages/openchat/src/server/projectors.ts`
- `packages/openchat/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/openchat/src/server/shared/fence.ts`
- `packages/openchat/src/session/message-v2.ts`
- `packages/openchat/src/session/projectors.ts`
- `packages/openchat/src/session/prompt.ts`
- `packages/openchat/src/session/session.ts`
- `packages/openchat/src/session/todo.ts`
- `packages/openchat/src/share/share-next.ts`
- `packages/openchat/src/storage/db.ts`
- `packages/openchat/src/sync/index.ts`
- `packages/openchat/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy openchat database wrapper; `packages/openchat/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the openchat wrapper.
...
Status: Completed. `SyncEvent` and the openchat projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy openchat database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy openchat database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy openchat database wrapper.
...
All migration groups are complete or superseded. `packages/openchat/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No openchat data-migration group remains.

The previous openchat `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate openchat data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/openchat`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/openchat/src`
- `bun typecheck` from `packages/openchat`
- Relevant package tests from `packages/openchat`, not the repo root
