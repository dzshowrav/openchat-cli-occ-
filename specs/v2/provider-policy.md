...
Both `action` and `resource` use openchat's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.openchat` files is intentionally deferred until `.openchat` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether openchat permits an operation through its managed execution paths.
