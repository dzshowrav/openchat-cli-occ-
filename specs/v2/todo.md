...
The openchat server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@openchat-ai/llm` tool loop after replacing its
...
- plugins get global "openchat" instance like in that post i showed
- openchat instance has stuff like `openchat.session.prompt()` or
  `openchat.tool.register({...})`
...
- decide whether to preserve deprecated `@openchat-ai/llm` orchestration exports
