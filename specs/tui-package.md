...
`packages/openchat/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/openchat ---\
                      > @openchat-ai/tui -> @openchat-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/openchat`,
`packages/cli`, or `@openchat-ai/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/openchat`.
- Do not replace private `packages/openchat` imports with `@openchat-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@openchat-ai/tui`.

## Ownership Boundary

### `@openchat-ai/tui` Owns
...
```text
packages/openchat/src/cli/cmd/tui
```

Its private dependency on `packages/openchat` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/openchat/src/*`.
...
Status: Completed. The private `@openchat-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@openchat-ai/tui`.
...
- The package has no dependency on `openchat`, `@openchat-ai/cli`, or
  `@openchat-ai/core`.
...
primitives now live in `@openchat-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@openchat-ai/tui/runtime`. Movable app, component,
...
`@openchat-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/openchat` initially.
...
canonical in `@openchat-ai/tui`. Configured references resolve through the new
...
`@openchat-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/openchat`.
...
- The package has no imports from `packages/openchat`, `packages/cli`, or
  `@openchat-ai/core`.
...
public `@openchat-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/openchat`.
- Keep the legacy embedded worker and server startup in `packages/openchat`.
- Change those adapters to load config, create transport inputs, and call the
  public `@openchat-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/openchat`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@openchat-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@openchat-ai/core`, or either
  executable package.
...
cd packages/openchat && bun typecheck
...
rg '@openchat-ai/core|packages/openchat|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/openchat/src packages/openchat/test
```
...
- Build the current-platform `packages/openchat` binary.
